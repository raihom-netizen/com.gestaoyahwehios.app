...
class FinanceAccountBalanceUtils {
  ...
  /// Saldo de abertura por conta (antes de [periodStart]) — mesma regra do período.
  ...
  static Map<String, double> mergeOpeningAndPeriodBalances(
  ...
  /// «Todas as contas» ou uma conta — abertura + líquido do período (paridade painel).
  static double consolidatedAccumulatedSaldo({
    required double openingTotal,
    required double periodNetTotal,
    String? accountFilterId,
    required Map<String, double> openingByAccount,
    Map<String, double>? periodNetByAccount,
    required double periodNetFallbackForAccount,
  }) {
    ...
    if (filterId != null && filterId.isNotEmpty) {
      ...
      return opening + periodNetFallbackForAccount;
    }
    return openingTotal + periodNetTotal;
  }
