...
String firestoreUserDocIdForAppShell(String passedFromShell) {
  ...
  // Mobile no shell autenticado: uid do widget é confiável (evita tela vazia pós-instalação).
  return passed;
}

/// Leitura de módulos — preferir sessão; no mobile usa [forAppShell] se strict vazio.
String firestoreUserDocIdForModuleReads(String shellUid) {
  final strict = firestoreUserDocIdStrictFromSession();
  if (strict.isNotEmpty) return strict;
  return firestoreUserDocIdForAppShell(shellUid);
}
