...
class MpCheckoutPricingSnapshot {
  MpCheckoutPricingSnapshot({
    ...
    this.extraBankConnectionMonthly = PremiumProLimits.extraConnectionMonthlyBrl,
  ...
  factory MpCheckoutPricingSnapshot.fromFirestore(Map<String, dynamic>? raw) {
    ...
    return MpCheckoutPricingSnapshot(
      premiumMonthly: _pickDouble(raw, 'premium_monthly') ?? d.premiumMonthly,
      premiumAnnual: _pickDouble(raw, 'premium_annual') ?? d.premiumAnnual,
      premiumProMonthly: _pickDouble(raw, 'premium_pro_monthly') ?? d.premiumProMonthly,
      premiumProAnnual: _pickDouble(raw, 'premium_pro_annual') ?? d.premiumProAnnual,
      extraBankConnectionMonthly: _pickDouble(raw, 'extra_bank_connection_monthly') ?? d.extraBankConnectionMonthly,
      extraBankConnectionAnnual: _pickDouble(raw, 'extra_bank_connection_annual') ?? d.extraBankConnectionAnnual,
    );
  }
  ...
  Map<String, String> generatedPremiumLandingFields() {
    ...
    return {
      ...
      'divPlanosSubtitle':
          'Plano Premium: finanças, metas e escalas num só lugar. Mensal ou anual — no anual sai cerca de $eq/mês em média; '
  ...
  List<({String code, String title, String price, String subtitle})> premiumPlanRowsForCheckout() {
    final pm = formatBrl(premiumMonthly);
    final pa = formatBrl(premiumAnnual);
    final eq = formatBrl(premiumAnnualEquivalentMonthlyFloor(premiumAnnual));
    return [
