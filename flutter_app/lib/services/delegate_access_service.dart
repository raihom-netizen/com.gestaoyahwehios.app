...
class DelegateAccessService {
  ...
  static DateTime? _lastLightRevalidateAt;
  static const Duration _lightRevalidateThrottle = Duration(minutes: 5);
  ...
  static Future<void> revalidateSession({bool blocking = false}) async {
    if (!blocking) {
      ...
      if (_lastLightRevalidateAt != null &&
          now.difference(_lastLightRevalidateAt!) < _lightRevalidateThrottle) {
        if (isActingAsDelegate) ensureDelegateIndexListener();
        return;
      }
      _lastLightRevalidateAt = now;
    }
    await loadFromPrefs();
    if (_cachedDataOwnerUid != null && !isActingAsDelegate) {
      await clear();
  ...
  static Future<void> _syncDelegateSessionProfile(
    ...
    try {
      ...
      if (!UserProfileEmailGuard.isValid(authEmail)) return;

      final ref = FirebaseFirestore.instance.collection('users').doc(sessionUid);
      final snap = await ref.get(const GetOptions(source: Source.serverAndCache));
      // Nunca criar users/{uid} sem e-mail — perfil delegate vem do AuthService.
      if (!snap.exists) return;

      await ref.set(
        UserProfileEmailGuard.safeMergePatch({
  ...
  static Future<String?> saveAuthorizedEmail({
    ...
    if (uid.isEmpty) {
      return 'Sessão inválida. Saia e entre novamente no app.';
    }
    ...
    try {
      ...
      return 'Não foi possível salvar o e-mail autorizado. Verifique a conexão e tente novamente.';
    }
  }
