...
const _subDir = 'yahweh_media_bytes_v1';
...
Future<Uint8List?> readYahwehMediaBytesDisk(String stableKey) async {
  ...
  try {
    ...
    if (DateTime.now().difference(stat.modified) >
        YahwehPerformanceV4.imageCacheStalePeriod) {
      try {
        await f.delete();
      } catch (_) {}
      return null;
...
Future<void> writeYahwehMediaBytesDisk(
  String stableKey,
  Uint8List bytes,
) async {
...
Future<void> deleteYahwehMediaBytesDisk(String stableKey) async {
  if (kIsWeb || stableKey.isEmpty) return;
  try {
    final root = await _dir();
    if (root == null) return;
    final f = File(p.join(root.path, _fileNameForStableKey(stableKey)));
...
Future<void> deleteYahwehMediaBytesDiskKeys(Iterable<String> keys) async {
  for (final k in keys) {
    if (k.isEmpty) continue;
    await deleteYahwehMediaBytesDisk(k);
  }
}
