...
import 'package:gestao_yahweh/utils/firestore_offline_write.dart';
import 'agenda_notification_reschedule_helper.dart';
import 'agenda_reminder_delete_helper.dart';
import 'external_calendar_reminder_hooks.dart';
...
class ExpressCompromissoAgendaSync {
  ...
  static Future<void> deleteScaleWithAgendaSync({
    ...
    try {
      ...
      beforeScaleData = snap.data();
      reminderId = reminderIdFromScaleDocId(scaleDocId);
      if (reminderId == null && beforeScaleData != null) {
        final linked = (beforeScaleData['linkedReminderId'] ?? '').toString();
        final agendaIdField = (beforeScaleData['agendaId'] ?? '').toString();
        reminderId = linked.isNotEmpty
            ? linked
      ...
      if (reminderId != null && reminderId.isNotEmpty) {
        try {
          beforeReminderData =
              (await _reminders(userDocId).doc(reminderId).get()).data();
        } catch (_) {}
      }
    } catch (_) {}
    ...
    PostSaveBackgroundCoordinator.schedule(
      ...
      widget: true,
      scaleDocId: scaleDocId,
      beforeScaleData: beforeScaleData,
      scaleDeleted: true,
      reminderDocId: reminderId,
      beforeReminderData: beforeReminderData,
      reminderDeleted: reminderId != null && reminderId.isNotEmpty,
    );
  }
  ...
  static Future<String> upsertFromExpress({
    ...
    await FirestoreOfflineWrite.set(
      reminderRef,
      payload,
    ...
    if (!deferBackground) {
      unawaited(
        AgendaNotificationRescheduleHelper.afterReminderSave(
          userDocId: userDocId,
    ...
    unawaited(
      ExternalCalendarReminderHooks.afterReminderUpsert(
        userDocId: userDocId,
        reminderDocId: reminderId,
        ...
        existingGoogleEventId:
            (beforeData?['googleEventId'] ?? '').toString().trim(),
      ),
    );
  ...
  static void refreshNotifications(String userDocId, {bool full = false}) {
    if (userDocId.isEmpty) return;
    NotificationCenterService.silentResyncAfterReconnect(userDocId);
    PostSaveBackgroundCoordinator.schedule(
      userDocId,
      notifications: true,
      widget: true,
      fullNotificationRefresh: full,
    );
  }
  ...
  static Future<int> upsertManyFromGeracaoAutomatica({
    ...
    for (var i = 0; i < items.length; i += batchLimit) {
      ...
      for (final item in chunk) {
        ...
        writes.add((
          ...
          data: {
            ...
            'title': title,
            'notes': '',
            'linkLocalizacao': item.linkLocalizacao.trim(),
            'contatoWhatsApp': item.contatoWhatsApp.trim(),
      ...
      await FirestoreOfflineWrite.setMany(items: writes, batchLimit: 450);
    }

    refreshNotifications(userDocId, full: true);
    return count;
  }
  ...
  static Future<int> deleteByMagicBatchIds({
    ...
    for (var i = 0; i < refs.length; i += batchLimit) {
      await FirestoreOfflineWrite.deleteMany(
        refs: refs.skip(i).take(batchLimit).toList(),
