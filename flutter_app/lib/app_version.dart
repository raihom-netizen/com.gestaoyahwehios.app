...
/// conformidade era a versao PUBLICADA na loja (API 35). Publicar este AAB em
/// producao encerra o aviso.
...
/// cabecalho fixo azul, o mesmo padrao ja usado no formulario; a rota deixou
/// tambem de mexer no URL do browser.
...
/// membro» no topo de Membros; card de finanças fora do painel inicial;
/// cabeçalho do site público centrado e sem cortar os dados da igreja.
/// 11.2.305+2135: Deploy completo — foto membro sync cartão, orações esteira, perf imagem CT (isolate+1600/78), patrimônio upload CT + botões Wisdom; regras/functions/índices/web/AAB/iOS + force update.
...
/// Storage. Foto membro — troca instantânea em lista/perfil/chat/cartão
/// (HTTPS bustado prioridade sobre path, bytes cache por revisão, PDF/cartão
/// invalidado). Cadastro público — FieldValue não quebra a callable
/// (encode cloud_firestore 6.x + strip CRIADO_EM); gyPublicSignupStatus
/// aceita PUBLIC_SIGNUP legado. Barra superior CT — sem foto, nome editável
/// (sheet moderno users/{uid}+Auth) + nome da igreja abaixo, barra discreta;
/// troféu e engrenagem removidos (Config última no rodapé); só sino + sair;
/// central de notificações com filtro/categoria Visitantes (live novos
/// visitantes p/ staff). Número do build só sobe no deploy autorizado.
/// 11.2.305+2121: Agenda + calendário Fornecedores = clone Escalas CT — botão Hoje, SAB/DOM/feriado vermelho, resumo do dia/mês, feriados, chips Localização/WhatsApp do cadastro; deploy completo.
/// 11.2.305+2120: Deploy completo — rodapé Cartão/Agenda/Membros, uploads/chat estáveis, permissões CRUD, self-only membros; regras/índices/functions/web/AAB/iOS.
/// 11.2.305+2119: Deploy completo — rodapé Cartão/Agenda/Membros, uploads/chat estáveis, permissões CRUD, self-only membros; regras/índices/functions/web/AAB/iOS.
/// 11.2.305+2118: Deploy completo — mídia sem hang + chat DM; anti-sumiço patrimônio/financeiro/fornecedores; limpeza BPC (membros+MP); regras/índices/functions/web/AAB/iOS.
/// 11.2.305+2117: Deploy completo — mídia sem hang + chat DM instantâneo; anti-sumiço patrimônio/financeiro/fornecedores; limpeza BPC (membros+MP); regras/índices/functions/web/AAB/iOS.
/// 11.2.305+2116: Mídia sem hang (~82%) — getDownloadURL soft-fail + path-only; YahwehChat DM instantâneo (Membros/aniversariantes); Master upload/display timeouts.
/// 11.2.305+2115: Deploy completo ? telas login/cadastro modernizadas, rodape linha unica; web 2114 online, AAB/iOS 2115.
/// 11.2.305+2114: Deploy completo — telas login/cadastro modernizadas (card único centralizado, logo no topo), rodapé linha única (Controle Total); web+AAB+iOS.
/// 11.2.305+2113: Web painel — semáforo reads 10+fila timeout, finance paralelo, Hive vazio não mascara, escalas sem hang, msg timeout leitura.
...
/// 11.2.305+2108: Deploy completo — upload nativo bytes=Web (eventos/avisos/chat/mídia), membro self-only, rodapé atalhos extras, Web/Master load rápido (repair BG, caps 14s).
/// 11.2.305+2107: Crashlytics iOS ? LateInit _deptsFuture + Json Infinity; menos reads membros (sem users listen); panel cache 60min.
...
/// 11.2.305+2078: Deploy completo ? gate CT unificado (DirectStorageUrlPublish), web 2078 online, AAB+iOS+push Codemagic.
/// 11.2.305+2077: Deploy completo — gate CT unificado (DirectStorageUrlPublish) em patrimônio/logo/membro/financeiro/chat finalize; regras/functions/índices/web/AAB/iOS.
...
/// 11.2.305+2042: Upload unificado DirectStorageUrlPublish ? bytes mobile em avisos/eventos; todos módulos (membro, patrim?nio, financeiro, logo, chat) mesmo gate Storage?URL.
...
/// 11.2.305+2039: Deploy completo — patrimônio/inventário paths+4 fotos, fornecedores cache-first, certificados/cartas/relatórios, regras/functions/web/AAB/iOS.
/// 11.2.305+2037: Deploy completo — remoção blindagens EcoFire (filas/recovery/strict), force update, regras/functions/web/AAB/iOS.
/// 11.2.305+2035: Deploy completo ? AppTheme, AppPublishActionRow (Aviso/Evento), chat repository, EcoFire retry UI, web/AAB/iOS.
...
/// 11.2.305+2009: Publicar aviso/evento — fim core/no-app (Storage relink + probe antes de upload; gate único).
/// 11.2.305+2008: Web ? aguarda repairMyChurchBinding (Completer); callable membros primeiro; cap 32s; reset ao trocar conta.
/// 11.2.305+2007: Reativa repairMyChurchBinding (permission-denied em todos módulos); bootstrap acesso painel.
/// 11.2.305+2005: Web — Membros via ChurchMembersLoadService (guard web + cache-first); fim erro «Não foi possível carregar os membros».
/// 11.2.305+1998: Deploy completo — tenant canónico síncrono igrejas/{churchId} (Firestore+Storage); cartão membro CNH PDF; bind imediato login/shell.
/// 11.2.305+1997: Hotfix — Firebase init antes runApp; painel abre com cache perfil/church (sem erro igreja); sessão web rápida.
/// 11.2.305+1991: Deploy completo — fotos membros rápidas, Termos/Privacidade tela cheia, patrimônio/chat/exclusão membro.
/// 11.2.305+1990: Deploy completo — Firestore web CT, cartão membro path igrejas/{id}/membros.
...
/// 11.2.305+1983: Paths centralizados igrejas/{churchId} ? shell desbloqueia módulos no 1.? frame; gateway ?nico por módulo; listCacheFirst com timeout.
...
/// 11.2.305+1901: Painel ? slug BPC?doc canónico síncrono (cadastro, departamentos, cargos); shell sem tenant errado no 1.? frame.
...
/// 11.2.295+1723: ChurchDataService ?nico ? Firestore?sucesso?upload?URL; queries limit 20; logs completos.
...
/// 11.2.295+1694: Painel rápido ? AuthGate cache 1.? frame, getUserProfile+igreja no servidor, sem bloquear repair.
...
/// 11.2.295+1659: Login persistente ? splash sess?o?painel; biometria só no AuthGate; preheat paralelo.
...
/// 11.2.295+1641: Chat Igreja ? conversas estáveis (?ndice nunca apaga com mensagens), entregue ??, mapa Firestore.
...
/// 11.2.295+1587: deploy completo ? Command Center master, login auto-sessão, chat 2? entrega.
...
/// 11.2.305+2004: Deploy completo ? alinhamento Web/Android/iOS (feed mural, financeiro, visitantes/cargos, master admin); tenant igrejas/{churchId}.
