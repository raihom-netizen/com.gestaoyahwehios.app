...
class _ChurchLettersPageState extends State<ChurchLettersPage>
  ...
  Future<void> _pickSigner({required bool second}) async {
    ...
    final picked = await showChurchLetterSignerPicker(
      context,
      title: second ? '2º assinante (opcional)' : '1º assinante *',
  ...
  Future<void> _emitPdf(_CartaKind kind, {bool saveHistorico = true}) async {
    ...
    if (_signer1MemberId == null || _signer1MemberId!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        ThemeCleanPremium.feedbackSnackBar(
          'Selecione o 1º assinante no cadastro de membros.',
        ),
      );
      return;
    }
                                        ...
                                        if (docs.isNotEmpty)
                                          InputDecorator(
                                            decoration: premiumField(
                                              ...
                                              helper:
                                                  'União dos contatos dos assinantes no cadastro',
                                            ),
                                            child: Text(
                                              _contactoPdf(
