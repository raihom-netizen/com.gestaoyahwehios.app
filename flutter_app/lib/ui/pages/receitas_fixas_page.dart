...
/// Receitas fixas (aluguel recebido, comissões, juros): lista por **categoria** + FAB; preferências de pendentes no ícone de afinação da AppBar.
class ReceitasFixasScreen extends StatefulWidget {
  final String uid;

  const ReceitasFixasScreen({super.key, required this.uid});

  @override
  State<ReceitasFixasScreen> createState() => _ReceitasFixasScreenState();
}

class _ReceitasFixasScreenState extends State<ReceitasFixasScreen> {
  final FixedIncomeService _service = FixedIncomeService();
  final FixedIncomePreferencesService _prefsService =
      FixedIncomePreferencesService();

  /// Padrão do app até o Firestore devolver as categorias do utilizador (evita dropdown só com «Incluir nova»).
  List<String> _incomeCategories = List<String>.from(kDefaultIncomeCategories)
    ..sort(UserCategoriesService.compareNamesPt);
  Future<List<Map<String, dynamic>>>? _fixedIncomesFuture;
  StreamSubscription<fa.User?>? _authUidSub;

  String get _fsUid => widget.uid.trim();

  /// Modelos de descrição (app) + depois mesclamos descrições já guardadas em `fixed_incomes`.
  static const List<String> _kPresetFixedIncomeDescriptions = [
    'Aluguel recebido',
    'Comissões',
    'Juros recebidos',
    'Salário',
  ...
  void _refreshFixedIncomes() {
    setState(() {
      _fixedIncomesFuture = _service.list(_fsUid);
    });
  }
  ...
  Future<void> _loadCategories() async {
    ...
    if (!mounted) return;
    setState(() {
      _incomeCategories = UserCategoriesService.sortedWithoutIncluirNova([
  ...
  static Widget _buildCategoryDropdown({
    ...
    final options = [kIncluirNova, ...incomeCategories];
    final value = category == kIncluirNova ||
            incomeCategories.contains(category)
        ? category
        : (incomeCategories.isNotEmpty ? incomeCategories.first : kIncluirNova);
  ...
  String _subtitleFixedIncome(
      Map<String, dynamic> e, int day, DateTime? start, DateTime? end,
      {bool includeCategory = true}) {
    final mode = (e['mode'] ?? FixedIncomeService.modePeriod).toString();
    final cat = e['category'] ?? 'Receita';
    final periodTail =
  ...
  Future<void> _openForm({Map<String, dynamic>? existing}) async {
    final fixedRows = await _service.list(_fsUid);
    if (!mounted) return;
    final descSuggestions = <String>{};
    for (final s in _kPresetFixedIncomeDescriptions) {
      descSuggestions.add(s);
    }
    for (final m in fixedRows) {
      final d = (m['description'] ?? '').toString().trim();
      if (d.length >= 2) descSuggestions.add(d);
    }
    final descriptionSuggestionsAll = descSuggestions.toList()
      ..sort(UserCategoriesService.compareNamesPt);
    final descriptionSuggestionsShown = descriptionSuggestionsAll.length > 32
        ? descriptionSuggestionsAll.sublist(0, 32)
    ...
    String category = existing?['category']?.toString() ??
        (_incomeCategories.isNotEmpty ? _incomeCategories.first : 'Receita');
                        ...
                        children: [
                          ...
                          if (descriptionSuggestionsShown.isNotEmpty) ...[
                            ...
                            Align(
                              ...
                              child: Text(
                                'Sugestões (modelos e já cadastradas) — toque para preencher ou escreva outra',
                                style: TextStyle(
                                    fontSize: 12,
                          ...
                          Container(
                            ...
                            child: SegmentedButton<String>(
                              segments: const [
                                ButtonSegment(
                                    value: FixedIncomeService.modePeriod,
                                    label: Text('Por período'),
                                    icon: Icon(Icons.date_range_rounded,
                                        size: 20)),
                                ButtonSegment(
                                    value: FixedIncomeService.modeInstallments,
                                ...
                                if (isEdit && id != null) {
                                    ...
                                    if (updatedCount > 0) {
                                      ScaffoldMessenger.of(context)
                                          .showSnackBar(SnackBar(
                                              content: Text(
                                                  'Receita fixa atualizada. $updatedCount parcela(s) futura(s) ajustada(s) para o novo dia.')));
                                    } else {
                                      ScaffoldMessenger.of(context)
                                  ...
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                          content: Text(
                                              'Receita fixa criada. Gerando lançamentos no Financeiro…')),
                                    );
                                  }
                                }
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
              ...
              offset: const Offset(0, 6)),
          BoxShadow(
              color: AppColors.financeReceita.withValues(alpha: 0.28),
              blurRadius: 10,
          ...
          if (items.isEmpty) {
            return Center(
              child: Padding(
                ...
                child: Text(
                  'Nenhuma receita fixa. Toque em «Nova receita fixa» para adicionar.',
                ...
                sliver: SliverList(
                  delegate: SliverChildBuilderDelegate(
                    (context, i) {
                      ...
                      final catName = (e['category'] ?? 'Receita').toString();
                      final vis =
                          financeCategoryVisualFor(catName, isIncome: true);
