...
import 'package:gestao_yahweh/models/smart_input_pop_result.dart';
import 'package:gestao_yahweh/models/user_profile.dart';
import 'package:gestao_yahweh/services/bank_notification_parser.dart';
import 'package:gestao_yahweh/services/finance_service.dart';
...
class SmartInputBatchPreviewScreen extends StatefulWidget {
  ...
  @override
  State<SmartInputBatchPreviewScreen> createState() =>
      _SmartInputBatchPreviewScreenState();
}
...
class _BatchRow {
  ...
  List<String> catsForType(List<String> inc, List<String> exp) =>
      isIncome ? inc : exp;
}
...
class _SmartInputBatchPreviewScreenState
    extends State<SmartInputBatchPreviewScreen> {
  ...
  String? _pickAccountId(BankNotificationParseResult p, bool isIncome) {
    ...
    if (def != null &&
        def.isNotEmpty &&
        widget.accounts.any((a) => a.id == def)) {
      return def;
    }
      ...
      _rows.add(
        _BatchRow(
          ...
          amountCtrl: TextEditingController(
              text: CurrencyFormats.formatBRLInput(r.valor)),
          ...
          accountMatchesSuggestedPreset:
              _accountMatchesPreset(aid, r.suggestedPresetId),
        ),
      );
    }
  ...
  ({double expense, double income, double pendingSelected})
      _breakdownSelected() {
    var expense = 0.0;
    var income = 0.0;
    var pending = 0.0;
  ...
  Future<void> _addCategory(_BatchRow row) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Não foi possível guardar a categoria.'),
    ...
    setState(() {
      if (isIncome) {
        if (!_incomeCats.contains(c)) {
          _incomeCats = [..._incomeCats, c]
            ..sort(UserCategoriesService.compareNamesPt);
        }
      } else {
        if (!_expenseCats.contains(c)) {
          _expenseCats = [..._expenseCats, c]
            ..sort(UserCategoriesService.compareNamesPt);
        }
      }
      row.category = c;
  ...
  List<_PersistJob> _buildPersistJobs(List<_BatchRow> selected) {
    ...
    for (final list in map.values) {
      ...
      final parts = list
          .map((r) => r.descCtrl.text.trim())
          .where((s) => s.isNotEmpty)
          .toList();
      var desc = parts.join(' | ');
      if (desc.length > 450) desc = '${desc.substring(0, 447)}…';
      var raw =
          list.map((r) => r.rawSnippet).where((s) => s.isNotEmpty).join('\n');
  ...
  Future<void> _offerBatchReceipt(List<String> txIds) async {
      ...
      builder: (ctx) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Comprovante para os ${txIds.length} lançamentos?',
                  style: Theme.of(ctx)
                      .textTheme
                      .titleMedium
              ...
              const Text(
                  'O mesmo ficheiro será associado a cada lançamento (opcional).',
    ...
    if (bytes.lengthInBytes > maxBytes) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Ficheiro demasiado grande (máx. 5 MB).')));
      }
      return;
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(okCount == txIds.length
                ? 'Comprovante associado a todos.'
                : 'Comprovante enviado para $okCount de ${txIds.length}.')),
      );
    }
  }
  ...
  Future<void> _saveSelected() async {
    ...
    if (selected.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Marque pelo menos um lançamento.')));
      return;
    }
    ...
    for (var i = 0; i < selected.length; i++) {
      ...
      if (amount == null || amount <= 0 || desc.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Linha ${i + 1}: valor ou descrição inválidos.'),
      ...
      if (desc.length < 3) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Linha ${i + 1}: histórico deve ter pelo menos 3 caracteres.'),
              backgroundColor: const Color(0xFFB00020)),
        );
        return;
      }
      if (row.category.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Linha ${i + 1}: escolha uma categoria.'),
              backgroundColor: const Color(0xFFB00020)),
        );
        return;
      }
      ...
      if (!row.isIncome &&
          (aid.isEmpty || !widget.accounts.any((a) => a.id == aid))) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Linha ${i + 1}: selecione a conta para despesa.'),
              backgroundColor: const Color(0xFFB00020)),
        );
        return;
      }
    ...
    try {
      ...
      if (mounted) {
        ...
        Navigator.of(context)
            .pop(SmartInputPopResult(createdTransactionIds: createdIds));
      }
    } finally {
      if (mounted) setState(() => _saving = false);
    }
  ...
  String _compactRowSubtitle(_BatchRow row) {
    ...
    final c =
        row.category.trim().isEmpty ? 'Sem categoria' : row.category.trim();
  ...
  int _confidenceScore(_BatchRow row) {
    ...
    if (!row.isIncome && ((row.financeAccountId ?? '').trim().isEmpty)) {
      score -= 12;
    }
    ...
    return Column(
      ...
      children: [
        if (row.possibleDuplicate ||
            _isSuspicious(row) ||
            (row.accountMatchesSuggestedPreset &&
                (row.suggestedPresetId ?? '').isNotEmpty)) ...[
          Wrap(
            spacing: 6,
            runSpacing: 6,
            children: [
              if (row.possibleDuplicate)
                Chip(
                  avatar: Icon(Icons.copy_all_rounded,
                      size: 14, color: Colors.orange.shade900),
                  label: const Text('Possível duplicado',
              ...
              if (row.accountMatchesSuggestedPreset &&
                  (row.suggestedPresetId ?? '').isNotEmpty)
                Chip(
                  avatar: Icon(Icons.verified_rounded,
                      size: 14, color: scheme.primary),
                  label: Text('Conta = sugestão (${row.suggestedPresetId})',
              ...
              if (_isSuspicious(row))
                Chip(
                  avatar: Icon(Icons.rule_folder_outlined,
                      size: 14, color: _confidenceColor(row)),
                  label: Text(
                  ...
                  padding: const EdgeInsets.symmetric(horizontal: 6),
                  backgroundColor:
                      _confidenceColor(row).withValues(alpha: 0.12),
                ),
            ],
          ),
          const SizedBox(height: 8),
            ...
            SegmentedButton<bool>(
              ...
              segments: const [
                ButtonSegment(
                    value: false,
                    label: Text('Despesa'),
                    icon: Icon(Icons.north_east_rounded, size: 13)),
                ButtonSegment(
                    value: true,
                    label: Text('Receita'),
                    icon: Icon(Icons.south_west_rounded, size: 13)),
              ],
              ...
              onSelectionChanged: (s) {
                ...
                setState(() {
                  ...
                  row.financeAccountId =
                      _pickAccountId(synthetic, row.isIncome);
                  row.accountMatchesSuggestedPreset = _accountMatchesPreset(
                      row.financeAccountId, row.suggestedPresetId);
                  _recomputeDuplicates();
                });
              },
            ),
                    ...
                    selectedItemBuilder: (ctx) => [
                      for (final c in sortedCats)
                        financeCategoryDropdownMenuRow(c,
                            isIncome: row.isIncome, isIncluirNovaOption: false),
                    ],
                    items: [
                      for (final c in sortedCats)
                        DropdownMenuItem(
            ...
            child: DropdownButton<String?>(
              ...
              onChanged: accounts.isEmpty && !row.isIncome
                  ...
                  : (v) => setState(() {
                        ...
                        row.accountMatchesSuggestedPreset =
                            _accountMatchesPreset(v, row.suggestedPresetId);
                      }),
            ),
          ),
        ),
  ...
  Widget _rowCard(_BatchRow row, ColorScheme scheme) {
    ...
    var sortedCats = List<String>.from(cats)
      ..sort(UserCategoriesService.compareNamesPt);
    if (row.category.trim().isNotEmpty && !sortedCats.contains(row.category)) {
      sortedCats = [...sortedCats, row.category]
        ..sort(UserCategoriesService.compareNamesPt);
    }
    final safeCat =
        row.category.trim().isNotEmpty && sortedCats.contains(row.category)
            ? row.category
    ...
    if (row.isIncome) {
      if (accountVal != null &&
          accountVal.isNotEmpty &&
          !accounts.any((a) => a.id == accountVal)) {
        accountVal = null;
      }
    } else {
      if (accounts.isEmpty) {
      ...
      child: Material(
        ...
        child: DecoratedBox(
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(14),
            border: Border.all(
                color: borderColor, width: row.listUiExpanded ? 1.25 : 1),
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      tooltip:
                          row.listUiExpanded ? 'Recolher' : 'Editar detalhes',
                      constraints:
                          const BoxConstraints(minWidth: 36, minHeight: 36),
                      padding: EdgeInsets.zero,
                      onPressed: () => setState(
                          () => row.listUiExpanded = !row.listUiExpanded),
                      icon: Icon(
                          row.listUiExpanded
              ...
              AnimatedCrossFade(
                ...
                secondChild: Padding(
                  ...
                  child: _rowExpandedBlock(
                      row, scheme, sortedCats, safeCat, accountVal, accounts),
                ),
                crossFadeState: row.listUiExpanded
  ...
  List<Widget> _batchScrollChildren(ColorScheme scheme) {
    ...
    for (final i in ordered) {
      ...
      if (_groupMode != SmartInputBatchGroupMode.lista &&
          header != lastHeader) {
        lastHeader = header;
        children.add(
          Padding(
  ...
  Widget _compactBatchToolbar(ColorScheme scheme) {
    final uncategorizedTotal =
        _rows.where((r) => r.category.trim().isEmpty).length;
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          SegmentedButton<SmartInputBatchGroupMode>(
            ...
            segments: const [
              ButtonSegment(
                  value: SmartInputBatchGroupMode.lista,
                  label: Text('Lista'),
                  icon: Icon(Icons.view_list_rounded, size: 15)),
              ButtonSegment(
                  value: SmartInputBatchGroupMode.porData,
                  label: Text('Data'),
          ...
          Row(
            ...
            children: [
              ...
              TextButton(
                ...
                child: const Text('Marcar sem cat.',
                    style:
                        TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
              ),
              TextButton(
          ...
          SegmentedButton<SmartBatchPersistMode>(
            ...
            segments: const [
              ButtonSegment(
                  value: SmartBatchPersistMode.umPorLinha,
                  label: Text('1 doc / linha')),
              ButtonSegment(
                  value: SmartBatchPersistMode.agruparIguais,
                  label: Text('Agrupar iguais')),
            ],
            ...
            : _rows.isEmpty
                ? Center(
                    child: Padding(
                      ...
                      child: Text('Nenhum lançamento válido.',
                      ...
                      Builder(
                        builder: (ctx) {
                          ...
                          return _SummaryBar(
                            ...
                            duplicateHints: _rows
                                .where((r) => r.selected && r.possibleDuplicate)
                                .length,
                            suspiciousCount: _rows
                                .where((r) => r.selected && _isSuspicious(r))
                                .length,
                          );
                        },
                      ),
