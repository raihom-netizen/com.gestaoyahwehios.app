...
class _DepartmentsPageState extends State<DepartmentsPage> {
      ...
      setState(() {
        ...
        if (mem != null && mem.docs.isNotEmpty) {
          ...
          _deptError ??= TimeoutException(
            'Tempo esgotado ao carregar departamentos. Toque em atualizar.',
          );
          _scheduleDepartmentsRetry();
        }
      });
    });
  }

  void _scheduleDepartmentsRetry() {
    final gen = ++_deptLoadRetryGen;
    for (final delay in const [2, 6, 14]) {
      Future<void>.delayed(Duration(seconds: delay), () async {
        if (!mounted || gen != _deptLoadRetryGen) return;
        if (_hydratedDeptDocs != null && _hydratedDeptDocs!.isNotEmpty) return;
        await _refreshDepartmentsBackground(forceRefresh: delay >= 6);
      });
    }
  }

  void _applyDepartmentsLoadResult(ChurchDepartmentsLoadResult result) {
    if (!mounted) return;
    if (result.docs.isNotEmpty) {
      ChurchDepartmentsLoadService.putRam(result.churchId, result.docs);
      unawaited(ChurchDepartmentsLoadService.persistAfterLoad(result));
    }
    setState(() {
      _deptLoading = false;
      ...
      if (result.docs.isNotEmpty) {
        ...
        _deptError = result.softError != null
            ...
            : StateError('Nenhum departamento encontrado.');
        _scheduleDepartmentsRetry();
      }
    });
    if (result.docs.isNotEmpty) {
  ...
  void _hydrateDepartmentsFromInstantCache() {
    ...
    final ram = ChurchDepartmentsLoadService.peekRam(seed);
  ...
  Future<QuerySnapshot<Map<String, dynamic>>> _seedOrLoadDepartments({
  ...
  Future<void> _openDepartmentsFast() async {
    ...
    try {
      ...
      if (!mounted) return;
      _applyDepartmentsLoadResult(result);
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _deptLoading = false;
  ...
  Future<void> _startDeptLoad({bool forceServer = false}) async {
    ...
    try {
      ...
      if (!mounted) return;
      _applyDepartmentsLoadResult(result);
      if (_memberLookupDone == false && result.docs.isNotEmpty) {
        unawaited(_loadMemberCpfLookup());
      }
    } catch (e, st) {
  ...
  Future<QuerySnapshot<Map<String, dynamic>>> _resolveTenantAndLoad({
    ...
    _bindPanelChurchId();
    var result = await ChurchDepartmentsLoadService.load(
      seedTenantId: widget.tenantId.trim(),
      forceRefresh: forceServer,
      forceServer: forceServer,
    );
  ...
  CollectionReference<Map<String, dynamic>> _departamentosCol(String tid) =>
      ChurchRepository.collection('departamentos', churchIdHint: tid);
    ...
    try {
      ...
      final result = await Navigator.of(context).push<Set<String>>(
        MaterialPageRoute<Set<String>>(
          ...
          builder: (ctx) => ChurchDepartmentMemberPickerPage(
            tenantId: _tid,
            deptId: deptId,
            deptName: deptName,
            accent1: accent1,
            accent2: accent2,
            mode: ChurchDepartmentMemberPickerMode.linkMembers,
          ),
        ),
      );
  ...
  CollectionReference<Map<String, dynamic>> get _col =>
      ChurchRepository.collection('departamentos', churchIdHint: _tid);

  CollectionReference<Map<String, dynamic>> get _membersCol =>
      ChurchRepository.collection('membros', churchIdHint: _tid);
  ...
  Widget _buildDepartmentCardImpl(
    ...
    final (c1, c2) = _deptCardGradientInts(m, resolvedIcon);
    final colorA = Color(c1);
    final colorB = Color(c2);
    final isArchived = !churchDepartmentDocIsActive(m);
    ...
    if (leaderCpfs.isNotEmpty && members.isNotEmpty) {
      for (final row in members) {
        final cpf = (row.data['CPF'] ?? row.data['cpf'] ?? '')
            .toString()
            .replaceAll(RegExp(r'\D'), '');
        final cpfNorm = cpf.length >= 9
            ? ChurchDepartmentLeaders.canonicalCpfDigits(cpf)
            : '';
        if (cpfNorm.isNotEmpty && leaderCpfs.contains(cpfNorm)) {
          leaderRow = row;
          break;
        }
      }
    ...
    final displayLeaderFoto = leaderFoto.isNotEmpty
        ? leaderFoto
        : (leaderRow != null ? imageUrlFromMap(leaderRow.data) : '');
    final displayLeaderMemberId = leaderRow?.memberDocId ?? '';

    void openHub() => _openDepartmentHubSheet(deptDoc: d);

    final primaryAvatar = _iconChip(resolvedIcon, radius: 24);

    Widget memberAvatarsFooter() {
      if (total == 0) {
        return Text(
          'Sem participantes',
          style: TextStyle(
      ...
      final stackW = 30.0 + (shown.length - 1) * 20.0 + (rest > 0 ? 30.0 : 0.0);
      return SizedBox(
        height: 34,
        width: stackW,
        child: Stack(
    ...
    Widget menuButton() {
      return PopupMenuButton<String>(
        tooltip: 'Opções do departamento',
        color: Colors.white,
        elevation: 10,
    ...
    return Padding(
      ...
      child: DecoratedBox(
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: colorA.withValues(alpha: isArchived ? 0.28 : 0.48),
            width: 1.15,
          ),
          boxShadow: _deptCardShadow,
        ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        if (displayLeaderName.isNotEmpty) ...[
                          Stack(
                            clipBehavior: Clip.none,
                            children: [
                              FotoMembroWidget(
                                size: 32,
                                tenantId: _tid,
                                memberId: displayLeaderMemberId,
                                imageUrl: displayLeaderFoto.isEmpty
                                    ? null
                                    : displayLeaderFoto,
                                memberData: leaderRow?.data ?? const {},
                                preferListThumbnail: true,
                                memCacheWidth: 96,
                              ),
                              Positioned(
                          ...
                          Expanded(
                            child: Text(
                              'Toque para abrir o departamento',
                              style: TextStyle(
                                fontSize: 12,
                                fontWeight: FontWeight.w600,
  ...
  Future<String?> _pickMemberCpfDigitsForLeader({
    String deptName = 'Departamento',
    int accent1 = 0xFF2563EB,
    int accent2 = 0xFF7C3AED,
  }) async {
    if (_tid.trim().isEmpty || !mounted) return null;
    return Navigator.of(context).push<String>(
      MaterialPageRoute<String>(
        ...
        builder: (ctx) => ChurchDepartmentMemberPickerPage(
          ...
          accent1: accent1,
          accent2: accent2,
          mode: ChurchDepartmentMemberPickerMode.singleLeader,
        ),
      ),
    );
  }
  ...
  Widget _buildDepartmentsHubFromHydrated({
    ...
    int welcomeOrder(QueryDocumentSnapshot<Map<String, dynamic>> d) {
      final m = d.data();
      if (m['isWelcomeKit'] == true) {
    ...
    if (docs.isEmpty && _tid.isNotEmpty && !_emptyServerDeptFetchTried) {
      _emptyServerDeptFetchTried = true;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) unawaited(_fetchDepartmentsFromServerOnce());
      });
    }
            ...
            Expanded(
              child: Builder(
                builder: (context) {
                  ...
                  return _buildDepartmentsHubFromHydrated(
                    context: context,
                    padding: padding,
...
class _DepartmentHubSheet extends StatefulWidget {
  ...
  @override
  State<_DepartmentHubSheet> createState() => _DepartmentHubSheetState();
}

class _DepartmentHubSheetState extends State<_DepartmentHubSheet> {
