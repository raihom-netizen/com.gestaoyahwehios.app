...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final membersCol = ChurchUiCollections.membros(
      _effectiveTenantId,
    ).limit(300);

    final visitantesMesQuery =
        ChurchUiCollections.visitantes(_effectiveTenantId)
            .where(
              'createdAt',
      ...
      void open() {
        if (title == 'Membros') {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (_) =>
                  MembersPage(tenantId: _effectiveTenantId, role: widget.role),
            ),
          );
          return;
        }
        ...
        if (title == 'Usuarios') {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (_) =>
                  UsersPage(tenantId: _effectiveTenantId, role: widget.role),
            ),
          );
          return;
        }
        ...
        if (title == 'Receitas e Despesas') {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (_) =>
                  FinancePage(tenantId: _effectiveTenantId, role: widget.role),
            ),
          );
          return;
        }
        ...
        if (title == 'Informações do Sistema') {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (_) =>
                  SistemaInformacoesPage(tenantId: _effectiveTenantId),
            ),
          );
          return;
        }
                      ...
                      builder: (context, cachedSnap) {
                        ...
                        >(
                          ...
                          builder: (context, membersSnap) {
                            ...
                            final totalNewIn6Months = newMembersByMonth
                            ...
                            for (var i = 0; i < newMembersByMonth.length; i++) {
                              ...
                              growthSpots.add(
                                FlSpot(i.toDouble(), cumul.toDouble()),
                              );
                              final m = monthStarts[i];
                              monthLabels.add(
                                  ...
                                  builder: (context, visitSnap) {
                                    ...
                                    >(
                                      ...
                                      builder: (context, eventSnap) {
                                        ...
                                        final nextEventDate =
                                            nextEvent['startAt'] is Timestamp
                                            ? (nextEvent['startAt']
                                                      as Timestamp)
                                                  .toDate()
                                            : null;
                                        final nextEventLabel =
                                            nextEventDate == null
                                            ? 'Sem data'
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, tenantSnap) {
        ...
        if (tenantSnap.hasData) {
          final billing =
              tenantSnap.data!.data()?['billing'] as Map<String, dynamic>?;
        ...
        return FutureBuilder<DocumentSnapshot<Map<String, dynamic>>?>(
          ...
          builder: (context, planSnap) {
            ...
            if (planSnap.hasData &&
                planSnap.data != null &&
                planSnap.data!.exists) {
              final data = planSnap.data!.data() ?? {};
              final raw =
                  data['membersMax'] ??
                  data['limits']?['members'] ??
                  data['members'];
              membersMax = raw is num
                  ? raw.toInt()
...
class _TopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final shortName = userName.length > 12
      ...
      child: Row(
        children: [
          ...
          SafeCircleAvatarImage(
            imageUrl: isValidImageUrl(photoUrl)
      ...
      content: SizedBox(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, i) {
            ...
            final cpf = ((b['cpf'] as String?) ?? '').replaceAll(
              RegExp(r'[^0-9]'),
              '',
            );
            return Row(
              children: [
                FotoMembroWidget(
                  imageUrl: isValidImageUrl(photo)
