...
import 'package:gestao_yahweh/constants/agenda_module_icons.dart';
...
/// Schema da preferência de tela inicial (2 = módulos separados Compromissos/Audiências).
const String kHomeDefaultStartModuleSchemaField = 'defaultStartModuleSchema';
const int kHomeDefaultStartModuleSchemaCurrent = 2;

/// Nome legado do módulo combinado (migração de textos antigos).
const String kAgendaModuleDisplayName = 'Audiências/Compromissos';

/// Rodapé muito estreito: mantém ícone no tamanho normal; só o texto abrevia.
const String kAgendaModuleFooterLabelNarrow = 'Aud./Comp.';

/// Resolve índice gravado em versões anteriores para o mapa atual do shell.
int migrateHomeStartModuleIndex(int raw) =>
    ShellModuleIndex.migrateLegacyIndex(raw);

int resolveHomeStartModuleIndex({
  required int raw,
  required int schema,
}) {
  if (schema >= kHomeDefaultStartModuleSchemaCurrent) {
    return kHomeDefaultStartModuleLabels.containsKey(raw)
        ? raw
        : ShellModuleIndex.inicio;
  }
  final migrated = migrateHomeStartModuleIndex(raw);
  return kHomeDefaultStartModuleLabels.containsKey(migrated)
      ? migrated
      : ShellModuleIndex.inicio;
}
...
const Map<int, String> kHomeDefaultStartModuleLabels = {
  ShellModuleIndex.inicio: 'Início (Painel inicial)',
  ShellModuleIndex.financeiro: 'Financeiro',
  ShellModuleIndex.escalas: 'Agenda/Escala',
  ShellModuleIndex.compromissos: 'Compromissos',
  ShellModuleIndex.audiencias: 'Audiências',
  ShellModuleIndex.calculadora: 'Calculadora',
};

/// Metadados visuais do seletor (ícone + subtítulo). Ordem = ordem na lista.
class HomeStartModulePickerOption {
  final int index;
  final IconData icon;
  ...
  const HomeStartModulePickerOption({
    required this.index,
    required this.icon,
...
const List<HomeStartModulePickerOption> kHomeStartModulePickerOptions = [
  HomeStartModulePickerOption(
    index: ShellModuleIndex.inicio,
    ...
    subtitle: 'Resumo financeiro, metas e atalhos do painel',
    shortLabel: 'Início',
  ),
  HomeStartModulePickerOption(
    index: ShellModuleIndex.financeiro,
    icon: Icons.account_balance_wallet_rounded,
    iconGradient: [AppColors.deepBlue, AppColors.primary],
    subtitle: 'Lançamentos, contas a pagar e a receber',
    shortLabel: 'Financeiro',
  ),
  HomeStartModulePickerOption(
    index: ShellModuleIndex.escalas,
    icon: Icons.event_note_rounded,
    iconGradient: [Color(0xFF1E3A5F), Color(0xFF2563EB)],
    subtitle: 'Agenda, plantões, locais e confirmações',
    shortLabel: 'Agenda/Escala',
  ),
  HomeStartModulePickerOption(
    index: ShellModuleIndex.compromissos,
    icon: AgendaModuleIcons.compromissoNav,
    iconGradient: [Color(0xFF1D4ED8), Color(0xFF6366F1)],
    subtitle: 'Compromissos particulares, lembretes e repetição anual',
    shortLabel: 'Compromissos',
  ),
  HomeStartModulePickerOption(
    index: ShellModuleIndex.audiencias,
    icon: AgendaModuleIcons.audienciaNav,
    iconGradient: [Color(0xFF5B21B6), Color(0xFF9333EA)],
    subtitle: 'Audiências, ofícios em anexo e prazos judiciais',
    shortLabel: 'Audiências',
  ),
  HomeStartModulePickerOption(
    index: ShellModuleIndex.calculadora,
    icon: Icons.calculate_rounded,
    iconGradient: [Color(0xFFEA580C), AppColors.amber],
    subtitle: 'Calculadora de banco de horas e plantões',
...
Future<void> showHomeStartModulePickerSheet(
  ...
  if (initialSelected != null &&
    ...
    try {
      ...
      final schema = snap.data()?[kHomeDefaultStartModuleSchemaField];
      final schemaNum = schema is num ? schema.toInt() : 1;
      current = resolveHomeStartModuleIndex(
        raw: raw is num ? raw.toInt() : 0,
        schema: schemaNum,
      );
      if (!kHomeDefaultStartModuleLabels.containsKey(current)) {
        current = ShellModuleIndex.inicio;
      }
    } catch (_) {
      current = ShellModuleIndex.inicio;
    }
                      ...
                      DecoratedBox(
                        ...
                        child: Padding(
                          ...
                          child: FilledButton.icon(
                            ...
                            onPressed: () async {
                              await saveHomeStartModulePreference(uid, current);
                              ...
                              if (context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content:
                                        Text('Tela inicial padrão atualizada.'),
...
class _HomeStartModuleTile extends StatelessWidget {
  final HomeStartModulePickerOption entry;
  final String title;
  final bool selected;
