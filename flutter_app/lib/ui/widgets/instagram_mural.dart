...
/// Envio: JPEG ~1080 px @ 75 % antes do upload ([evento_aviso_media_policy.dart]).
...
class InstagramMuralState extends State<InstagramMural> {
  /// FAB «Novo aviso» na [MuralPage] (shell).
  ...
  String _feedSearchQueryApplied = '';
  ...
  Query<Map<String, dynamic>> _feedBaseQuery({bool filtered = true}) {
    if (kIsWeb) {
      return _avisos.limit(_feedPageSize);
    }
    if (!filtered) {
  ...
  Future<void> _startFeedLiveSync() async {
    ...
    _feedLiveSub = ChurchTenantResilientReads.querySnapshotsResilient(
      ...
      if (!mounted) return;
      final liveDocs = _filterFeedDocs(snap.docs);
  ...
  bool _docMatchesFeedSearch(Map<String, dynamic> data) {
    final q = _feedSearchQueryApplied.trim().toLowerCase();
  ...
  Widget _buildFeedBody() {
    ...
    if (rawDocs.isNotEmpty) {
      final warmupKey = rawDocs.take(8).map((d) => d.id).join(',');
      if (warmupKey != _lastFeedMediaWarmupKey) {
        _lastFeedMediaWarmupKey = warmupKey;
        WidgetsBinding.instance.addPostFrameCallback((_) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        if (i < docs.length) return postAt(i);
  ...
  void _onFeedSearchChanged(String value) {
    _feedSearchQuery = value;
    _feedSearchDebounce?.cancel();
    _feedSearchDebounce = Timer(const Duration(milliseconds: 350), () {
      if (!mounted) return;
      setState(() => _feedSearchQueryApplied = _feedSearchQuery);
    });
  }
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                final searchField = TextField(
                  ...
                  onChanged: _onFeedSearchChanged,
...
class _PostCardState extends State<_PostCard>
  ...
  @override
  bool get wantKeepAlive => widget.feedIndex < 6;
  ...
  Widget build(BuildContext context) {
    ...
    final hasPhotoMedia =
        muralPhotos.isNotEmpty || eventNoticiaDocHasPhotoMedia(data);
    ...
    final mediaUploading =
        publishState == MuralFastPublishService.stateUploading && !hasPhotoMedia;
                  ...
                  if (publishFailed) {
                    return AspectRatio(
                      ...
                      child: ClipRRect(
                        ...
                        child: ColoredBox(
                          color:
                              ThemeCleanPremium.error.withValues(alpha: 0.06),
                          child: Padding(
...
class _MuralAvisoEditorPageState extends State<MuralAvisoEditorPage> {
  ...
  /// Web: bytes em RAM. Mobile: paths no disco (evita OOM no iPhone).
