...
class FinanceAccountCategorySheet extends StatefulWidget {
  ...
  final Future<void> Function(BuildContext context, String docId,
      Map<String, dynamic> current, String type) onEditTransaction;
  final Future<void> Function(BuildContext context, String docId)
      onDeleteTransaction;
  ...
  @override
  State<FinanceAccountCategorySheet> createState() =>
      _FinanceAccountCategorySheetState();
}

class _FinanceAccountCategorySheetState
    extends State<FinanceAccountCategorySheet> {
  /// 0 = todos (despesas + receitas), 1 = despesas, 2 = receitas — padrão: todos.
  int _tab = 0;
  ...
  bool _belongsToAccount(Map<String, dynamic> d) {
    ...
    return (d['financeAccountId'] ?? '').toString().trim() ==
        widget.account!.id;
  }
                  ...
                  builder: (context) {
                    if (_periodLoadError != null) {
                      return ListView(
                        ...
                        children: [
                          ...
                          SizedBox(height: 12),
                          Text('Erro ao carregar: $_periodLoadError',
                    ...
                    return FutureBuilder<
                      ...
                      builder: (context, openSnap) {
                        final openingTotal = openSnap.data?.total ??
                        ...
                        return ListView(
                          controller: scrollController,
                          padding:
                              EdgeInsets.fromLTRB(16, 4, 16, 16 + bottomInset),
                          children: [
                            ...
                            if (widget.account != null)
                              FilledButton.tonalIcon(
                                onPressed: () {
                                  ...
                                  widget
                                      .onApplyAccountFilter(widget.account!.id);
                                },
                                icon: Icon(Icons.filter_alt_rounded,
                                    size: 20),
                                label:
                                    Text('Filtrar lista só desta conta'),
  ...
  Widget _previewStrip(List<QueryDocumentSnapshot<Map<String, dynamic>>> raw) {
    final preview = (raw.toList()
          ..sort((a, b) => _docSortMs(b).compareTo(_docSortMs(a))))
        .take(3)
        .toList();
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              final amt = (d['amount'] ?? 0).toDouble();
              final desc =
                  (d['description'] ?? d['category'] ?? '').toString().trim();
              final instant = FinanceFaturaTransactionSort.effectiveInstant(d);
              final dateStr = instant != null
                  ? DateTimeFormats.formatDateTimeMinute(instant)
                  : '—';
              return Container(
                width: 168,
                padding: const EdgeInsets.all(12),
                decoration: context.appPanelDecoration(radius: 16, borderAccent: accent),
                child: Column(
          ...
          if (_gridSelectionMode)
            Wrap(
              ...
              children: [
                ...
                OutlinedButton.icon(
                  onPressed: _gridSelectedIds.isEmpty
                      ? null
                      : () => setState(() => _gridSelectedIds.clear()),
                  icon: Icon(Icons.deselect_rounded, size: 18),
                  label: Text('Nenhum'),
                ),
  ...
  List<Widget> _buildTransactionGrid(
      List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) {
...
List<_CatAgg> _aggregateByCategory(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) {
  ...
  for (final doc in docs) {
    ...
    final amt = (d['amount'] ?? 0).toDouble();
    final signed =
        (d['type'] ?? 'expense').toString() == 'expense' ? amt.abs() : amt;
    sums[key] = (sums[key] ?? 0) + signed;
  }
  final list = sums.entries
      .map((e) => _CatAgg(category: e.key, total: e.value, docs: m[e.key]!))
      .toList();
        ...
        child: Column(
          children: [
            ...
            Text(
              consolidated
                  ? (type == 'expense'
                      ? 'Nenhuma despesa no período consolidado.'
                      : 'Nenhuma receita no período consolidado.')
                  : (type == 'expense'
                      ? 'Nenhuma despesa nesta conta no período.'
                      : 'Nenhuma receita nesta conta no período.'),
