...
class ShiftLocation {
  /// Pré-cadastro gera lançamento de **escala/plantão** (padrão).
  static const String entryKindEscala = 'escala';

  /// Pré-cadastro gera **compromisso particular** (Agenda + calendário).
  static const String entryKindCompromisso = 'compromisso';
  ...
  /// `escala` = plantão na Escalas (padrão). `compromisso` = compromisso particular.
  final String entryKind;

  /// Link de localização padrão (Google Maps) — usado quando o pré-cadastro é
  /// **compromisso particular** e o dia é lançado a partir dele.
  final String linkLocalizacao;

  /// Contato WhatsApp padrão — idem (só tipo compromisso).
  ...
  const ShiftLocation({
    ...
    this.entryKind = entryKindEscala,
    this.linkLocalizacao = '',
    this.contatoWhatsApp = '',
  });

  bool get isEscalaTemplate => entryKind != entryKindCompromisso;

  bool get isCompromissoTemplate => entryKind == entryKindCompromisso;
  ...
  ShiftLocation copyWith({
    ...
    return ShiftLocation(
      ...
      linkLocalizacao: linkLocalizacao ?? this.linkLocalizacao,
      contatoWhatsApp: contatoWhatsApp ?? this.contatoWhatsApp,
    );
  }
  ...
  /// Remove sufixo de horário " HH:MM às HH:MM" e " N HORAS" do nome, se existir.
  ...
  /// Total de horas do turno (relógio AC4 padrão).
  static double totalHoursFromSchedule(String startTime, String endTime) {
    final (hd, hn) = hoursDayNightFromScaleClock(
      startHHmm: startTime.trim().isNotEmpty ? startTime.trim() : '08:00',
      endHHmm: endTime.trim().isNotEmpty ? endTime.trim() : '18:00',
    );
    return hd + hn;
  }
  ...
  static String fullNameWithSchedule(String baseName, String startTime, String endTime) {
    ...
    final baseUpper =
        base.isEmpty ? 'PLANTÃO' : uppercaseLatinPreservingEmoji(base);
    return '$baseUpper $start ÀS $end';
  }

  /// Ex.: "CASE DIURNO 07:00 ÀS 19:00 12 HORAS" — lista recorrente e seleção.
  static String fullNameWithScheduleAndHours(
    String baseName,
    String startTime,
    String endTime,
  ) {
    final withSchedule = fullNameWithSchedule(baseName, startTime, endTime);
    final hours = totalHoursFromSchedule(startTime, endTime);
    if (hours <= 0.009) return withSchedule;
    return '$withSchedule ${formatHorasDestaqueUi(hours)}';
  }
  ...
  static ShiftLocation fromMap(String? id, Map<String, dynamic>? data) {
    ...
    var nameRaw = uppercaseLatinPreservingEmoji(
      (data['name'] ?? '').toString().trim(),
    );
    if (nameRaw.isEmpty) {
      final fall = uppercaseLatinPreservingEmoji(
        (data['label'] ?? data['title'] ?? '').toString().trim(),
      );
      if (fall.isNotEmpty) nameRaw = fall;
    }
    var abbrRaw = uppercaseLatinPreservingEmoji(
      (data['abbreviation'] ?? '').toString().trim(),
    );
    ...
    return ShiftLocation(
      ...
      entryKind: _parseEntryKind(data['entryKind']),
      linkLocalizacao: (data['linkLocalizacao'] ?? '').toString().trim(),
      contatoWhatsApp: (data['contatoWhatsApp'] ?? '').toString().trim(),
    );
  }

  static String _parseEntryKind(dynamic v) {
    if (v == null) return entryKindEscala;
    final s = v.toString().trim().toLowerCase();
    if (s == entryKindCompromisso ||
        s == 'compromisso_particular' ||
        s == 'particular') {
      return entryKindCompromisso;
    }
    return entryKindEscala;
  }

  static String entryKindLabel(String kind) {
    switch (kind) {
      case entryKindCompromisso:
        return 'Compromisso';
      case entryKindEscala:
      default:
        return 'Escala';
    }
  }
