...
class UserProfile {
  ...
  static String composeDisplayNameParts(String first, String last) {
    final parts = [first.trim(), last.trim()].where((s) => s.isNotEmpty);
    return parts.join(' ').trim();
  }

  static String _resolveProfileNameFromFirestore(Map<String, dynamic> d) {
    final fn =
        (d['displayFirstName'] ?? d['firstName'] ?? '').toString().trim();
    final ln = (d['displayLastName'] ?? d['lastName'] ?? '').toString().trim();
    final composed = composeDisplayNameParts(fn, ln);
  ...
  factory UserProfile.fromFirestoreMap(String uid, Map<String, dynamic> d) {
    ...
    return UserProfile(
      ...
      name: _resolveProfileNameFromFirestore(d),
      firstName:
          (d['displayFirstName'] ?? d['firstName'] ?? '').toString().trim(),
      lastName: (d['displayLastName'] ?? d['lastName'] ?? '').toString().trim(),
  ...
  /// Acesso ao painel admin: role admin ou master.
