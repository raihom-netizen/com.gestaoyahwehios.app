...
/// Diurno: 05h01–21h59 | Noturno: 22h00–05h00 (Portaria SSP 621/2026)
///
...
class ScaleRates {
  ...
  double diurnoForWeekday(int weekday) => valueDiurno[weekday.clamp(0, 6)];

  double noturnoForWeekday(int weekday) => valueNoturno[weekday.clamp(0, 6)];
  ...
  /// Minuto noturno AC4 (padrão GO): 22h00 em diante e antes de 05h00.
  /// Diurno residual: 05h00–21h59 (05h01+ na prática; 05h00 entra como diurno).
  static bool _isNightMinute(
    int minuteOfDay,
    int nightStartMin,
    int nightEndMin,
  ) =>
  ...
  static Map<String, double> computeShiftWithRatesForMinute({
    ...
    double dayMin = 0,
        ...
        nightValue = 0;
    var cur = start;
    final endDt = end.isAfter(start) ? end : end.add(const Duration(days: 1));
  ...
  static ScaleRates fromMap(Map<String, dynamic>? data) {
    ...
    final parsed = ScaleRates(
      ...
      valueDiurno: parseRateList(data['valueDiurno']).length == 7
          ? parseRateList(data['valueDiurno'])
          : _defaultDiurno,
      valueNoturno: parseRateList(data['valueNoturno']).length == 7
          ? parseRateList(data['valueNoturno'])
          : _defaultNoturno,
