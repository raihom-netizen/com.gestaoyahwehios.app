...
/// Resultado do match plantão × pré-cadastro. [strong] = vínculo confiável
/// (nome base ou sigla exata); match só por sigla-substring é fraco e não pode
/// mudar classificação financeira (extra × ordinário) nem gravar valores.
class ShiftLocationMatch {
  final ShiftLocation location;
  final bool strong;
  const ShiftLocationMatch(this.location, this.strong);
}

/// Pré-cadastro que corresponde ao plantão, por prioridade:
/// 1. nome base idêntico; 2. contenção de nome; 3. sigla exata;
/// 4. sigla contida no rótulo (fraco — só quando nada mais casa).
///
/// **Não** prioriza pré-cadastro com financeiro ativo: a classificação
/// extra × ordinário vem do `financialEnabled` do pré-cadastro CORRETO.
/// (Bug 49.57: abbr "PLANTÃ" de pré-cadastro financeiro "roubava" o match de
/// "PLANTÃO ORD ..." e plantões ordinários viravam horas extras.)
ShiftLocationMatch? matchShiftLocationForScaleEntryDetailed(
    ScaleEntry e, List<ShiftLocation> locations) {
  final labelBase = normalizeLabelForMatch(e.label ?? '');
  final abbr = normalizeLabelForMatch(e.abbreviation ?? '');
  ...
  for (final loc in locations) {
    final nameBase =
        normalizeLabelForMatch(ShiftLocation.baseNameFromFull(loc.name));
    final locAbbr = normalizeLabelForMatch(loc.abbreviation);
    var score = 0;
    if (nameBase.isNotEmpty && labelBase == nameBase) {
      score = 1000;
    } else if (nameBase.isNotEmpty &&
        (labelBase.contains(nameBase) || nameBase.contains(labelBase))) {
      score = 100;
      if (labelBase.contains(nameBase)) score += 20;
    }
    if (locAbbr.isNotEmpty && abbr.isNotEmpty && abbr == locAbbr) {
      score += 50;
    } else if (score == 0 &&
        locAbbr.isNotEmpty &&
        labelBase.contains(locAbbr)) {
      score = 1;
    }
    if (score <= 0) continue;
    if (score > bestScore) {
  ...
  return ShiftLocationMatch(best, bestScore >= 50);
}

/// Mesma regra da tela Escalas / painel (compatibilidade com chamadas antigas).
ShiftLocation? matchShiftLocationForScaleEntry(
        ScaleEntry e, List<ShiftLocation> locations) =>
    matchShiftLocationForScaleEntryDetailed(e, locations)?.location;
...
class ScaleEntry {
  ...
  /// Link / endereço de localização (compromisso particular).
  final String? linkLocalizacao;

  /// Contato WhatsApp (número ou wa.me) — compromisso particular.
  ...
  /// Compromisso particular efetivo (exclui plantão/escala sem financeiro mal flagado).
  bool get isCompromissoParticularEfetivo =>
      scaleEntryIsCompromissoParticular(this);

  /// Audiência (módulo Agenda / espelho `agenda_*` com tipo audiencia).
  bool get isAudienciaEfetiva => scaleEntryIsAudienciaEfetiva(this);

  /// Plantão ou serviço de escala — não compromisso particular da Agenda.
  bool get isPlantaoEscala => scaleEntryIsPlantaoEscala(this);

  /// Mesma regra do painel inicial: vínculo Estado/Município/Particular **e** valor > 0.
  bool get temFinanceiroHabilitadoNoPainel {
    if (isCompromissoParticularEfetivo) return false;
  ...
  bool temFinanceiroPainelComLocais(List<ShiftLocation> locations) {
    ...
    // Vínculo + valor já gravados na escala (reparo ou lançamento expresso).
    if (temFinanceiroHabilitadoNoPainel) return true;
    if (locations.isNotEmpty) {
      // Só match forte decide financeiro; match fraco (sigla-substring) não
      // pode transformar plantão ordinário em extra nem o contrário.
      final match = matchShiftLocationForScaleEntryDetailed(this, locations);
      if (match != null && match.strong) return match.location.financialEnabled;
    }
    return false;
  }
  ...
  Map<String, dynamic> toMap() => {
        ...
        'totalValue': totalValue,
        'notes': notes ?? '',
        if (linkLocalizacao != null && linkLocalizacao!.trim().isNotEmpty)
          'linkLocalizacao': linkLocalizacao!.trim(),
        if (contatoWhatsApp != null && contatoWhatsApp!.trim().isNotEmpty)
        ...
        if (numeroOcorrencia != null && numeroOcorrencia!.isNotEmpty)
          'numeroOcorrencia': numeroOcorrencia,
        if (agendaType != null && agendaType!.isNotEmpty)
          'agendaType': agendaType,
  ...
  static ScaleEntry fromDoc(DocumentSnapshot<Map<String, dynamic>> doc) {
    ...
    return ScaleEntry(
      ...
      linkLocalizacao: () {
        final v = (d['linkLocalizacao'] ?? '').toString().trim();
        return v.isEmpty ? null : v;
      }(),
      contatoWhatsApp: () {
        final v = (d['contatoWhatsApp'] ?? '').toString().trim();
        return v.isEmpty ? null : v;
      }(),
      scaleNumber: isMirror
          ...
          : (scaleNumberRaw.isEmpty ? null : scaleNumberRaw),
      numeroSei:
          isMirror && mirrorNumeros!.sei.isNotEmpty ? mirrorNumeros.sei : null,
      numeroOcorrencia:
          isMirror && mirrorNumeros!.oco.isNotEmpty ? mirrorNumeros.oco : null,
