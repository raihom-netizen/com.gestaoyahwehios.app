...
exports.scheduledCleanupOrphanFiles = void 0;
...
function storagePathFields(data) {
    const out = new Set();
    const singles = [
        "storagePath",
        "logoPath",
 ...
 */
async function cleanupChurchOrphans(churchId, stats) {
    ...
    for (const mod of modules) {
        const snap = await churchRef
            ...
            .get()
            .catch(async () => churchRef.collection(mod.col).limit(DOCS_PER_MODULE).get());
...
exports.scheduledCleanupOrphanFiles = functions
    ...
    for (let page = 0; page < 8; page++) {
        ...
        if (snap.size < CHURCH_BATCH)
            break;
    }
    functions.logger.info("scheduledCleanupOrphanFiles", stats);
    return stats;
});
//# sourceMappingURL=cleanupOrphanFiles.js.map