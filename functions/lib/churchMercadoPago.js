...
function getFetch() {
    ...
    if (!f)
        throw new Error("fetch nao disponivel");
    return f;
}
...
async function getChurchMpAccessToken(tenantId) {
...
function appendTenantToNotificationUrl(url, tenantId) {
    const u = url.trim();
    if (!u || !tenantId)
        return u;
...
function computeMpNetAmounts(payment) {
    ...
    for (const f of feeDetails) {
        feesSum += Math.abs(Number(f?.amount ?? 0));
    }
...
function resolveDonorDisplayName(payment, meta) {
    const fromMeta = String(meta?.donorName ??
        meta?.donor_full_name ??
        meta?.donorFullName ??
        meta?.full_name ??
        meta?.payer_name ??
        "").trim();
    if (fromMeta && fromMeta.toLowerCase() !== "doador")
        return fromMeta.slice(0, 160);
    ...
    const dm = desc.match(/Doa[cç][aã]o\s*[—\-]\s*(.+)/i) || desc.match(/Doação\s*[—\-]\s*(.+)/i);
    if (dm?.[1]) {
        const t = dm[1].trim();
        if (t && t.toLowerCase() !== "doador")
            return t.slice(0, 160);
    }
    const add = payment?.additional_info;
    if (add && typeof add === "object") {
        const pAdd = add.payer;
        if (pAdd && typeof pAdd === "object") {
            ...
            const c2 = `${f2} ${l2}`.trim();
            if (c2)
                return c2.slice(0, 160);
            const n2 = String(pa.name ?? "").trim();
            if (n2)
                return n2.slice(0, 160);
        }
    }
    const email = String(payer.email ?? "").trim();
    if (email.includes("@")) {
        const local = (email.split("@")[0] ?? "").trim();
        if (local && !/^doacao\+/i.test(local))
 ...
 */
async function lancarTaxaComoDespesa(tenantId) {
    try {
        const snap = await fs()
            .collection("igrejas")
            .doc(tenantId)
 ...
 */
async function ensureMercadoPagoContaForNewChurch(tenantId) {
...
async function mpPostWithToken(accessToken, path, body, extraHeaders) {
    const res = await getFetch()(`https://api.mercadopago.com${path}`, {
        method: "POST",
        headers: {
            Authorization: `Bearer ${accessToken}`,
...
async function tryHandleChurchDonationPayment(payment) {
    ...
    const descricaoDoacao = buildDonationFinanceDescription(donationKind, donorName, memberFull, donationObs);
    await ref.set({
    ...
    let methodKey = "outro";
    if (ptRaw.includes("pix") || ptRaw === "account_money")
        methodKey = "pix";
    else if (ptRaw.includes("credit") || ptRaw.includes("debit") || ptRaw.includes("card"))
        methodKey = "cartao";
    await fs()
        .collection("igrejas")
        .doc(tenantId)
...
async function fetchPaymentForWebhook(dataId, platformMpGet, tenantHint) {
...
exports.saveChurchMercadoPagoCredentials = functions
    .region("us-central1")
    ...
    if (notificationWebhookUrl && !/^https:\/\//i.test(notificationWebhookUrl)) {
        throw new functions.https.HttpsError("invalid-argument", "URL do webhook deve comecar com https://");
    }
...
exports.ensureChurchTreasuryAccountPresets = functions
    .region("us-central1")
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Login necessario");
    ...
    for (const p of PRESET_CONTAS) {
        ...
        await col.doc("mercado_pago").set({
            ...
            numeroConta: "",
            tipoConta: "corrente",
            observacao: "Recebimentos via integração Mercado Pago. Configure o Access Token em Configurações → Contribuições / Mercado Pago.",
            ativo: true,
...
exports.createChurchDonationPix = functions
    .region("us-central1")
    .https.onCall(async (data, context) => {
    const tenantId = String(data?.tenantId || "").trim();
    ...
    if (!token) {
        throw new functions.https.HttpsError("failed-precondition", "Mercado Pago da igreja nao configurado. Peça ao gestor para informar o Access Token em Configuracoes.");
    }
    ...
    const email = payerEmail && payerEmail.includes("@")
        ? payerEmail
 ...
 */
exports.createChurchDonationPreference = functions
    .region("us-central1")
...
exports.pruneContribuicoesDizimoHistorico = functions
    .region("us-central1")
...
//# sourceMappingURL=churchMercadoPago.js.map