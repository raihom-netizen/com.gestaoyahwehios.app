...
exports.panelLoginUrl = panelLoginUrl;
exports.buildAvisoEmail = buildAvisoEmail;
exports.buildEscalaEmail = buildEscalaEmail;
exports.buildEventoEmail = buildEventoEmail;
exports.buildAniversarianteEmail = buildAniversarianteEmail;
exports.sendGestaoYahwehHtmlEmail = sendGestaoYahwehHtmlEmail;
/**
...
const FOOTER_VERSE = "Consagre ao Senhor tudo o que você faz, e os seus planos serão bem-sucedidos. — Provérbios 16:3";
...
function wrapEmail(opts) {
    ...
    const gradient = (0, notificationBranding_1.emailHeaderGradient)(mod);
    const badge = escapeHtml((0, notificationBranding_1.emailModuleBadgeLabel)(mod));
    const logo = (0, notificationBranding_1.gestaoBrandLogoUrl)();
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" style="max-width:560px;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 12px 40px rgba(15,23,42,0.08);border:1px solid #E2E8F0;">
        ...
        <tr><td style="padding:8px 22px 28px;" align="center">
          <a href="${cta}" style="display:inline-block;background:${BRAND_CTA};color:#fff!important;text-decoration:none;padding:14px 26px;border-radius:12px;font-weight:700;font-size:15px;">${escapeHtml(opts.ctaLabel)}</a>
        </td></tr>
        <tr><td style="padding:0 22px 22px;">
          <p style="margin:0;font-size:12px;color:${TEXT_MUTED};line-height:1.45;font-style:italic;text-align:center;border-top:1px solid #E2E8F0;padding-top:16px;">${escapeHtml(FOOTER_VERSE)}</p>
...
/** 2 — Escala / serviço */
function buildEscalaEmail(opts) {
...
/** 3 — Eventos */
function buildEventoEmail(opts) {
    const nome = escapeHtml(opts.memberName.trim() || "Olá");
...
function buildAniversarianteEmail(opts) {
...
async function sendGestaoYahwehHtmlEmail(opts) {
...
//# sourceMappingURL=memberNotificationEmail.js.map