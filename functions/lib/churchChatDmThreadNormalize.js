...
exports.repairChurchChatDmThreads = exports.backfillChurchChatDmThreads = exports.onChurchChatMessageIndexThread = exports.onChurchChatDmThreadWrite = void 0;
exports.parseDmThreadParticipants = parseDmThreadParticipants;
exports.repairDmThreadsForTenant = repairDmThreadsForTenant;
const functions = __importStar(require("firebase-functions/v1"));
const admin = __importStar(require("firebase-admin"));
const db = admin.firestore();
/** `dm_{uidMenor}_{uidMaior}` — UIDs Firebase não contêm `_`. */
function parseDmThreadParticipants(threadId) {
...
async function patchesForDmThread(threadId, data, threadRef) {
    ...
    const hasBoth = Array.isArray(current) &&
...
async function repairDmThreadsForTenant(tenantId) {
    const snap = await db
        .collection("igrejas")
        .doc(tenantId)
...
exports.onChurchChatMessageIndexThread = functions
    ...
    if (threadId.startsWith("dm_")) {
        const parsed = parseDmThreadParticipants(threadId);
        if (parsed)
            patches.participantUids = parsed;
        patches.type = "dm";
    }
    try {
...
exports.backfillChurchChatDmThreads = functions.pubsub
...
exports.repairChurchChatDmThreads = functions
    .region("us-central1")
    ...
    const body = (data || {});
    const { resolveTenantIdForCallable } = await Promise.resolve().then(() => __importStar(require("./tenantCallableResolve")));
...
//# sourceMappingURL=churchChatDmThreadNormalize.js.map