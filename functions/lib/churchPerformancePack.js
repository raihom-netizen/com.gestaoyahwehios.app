...
async function processMemberProfile(tenantId, memberId, srcPath) {
    ...
    const [thumbBuf, mediumBuf] = await Promise.all([
        (0, sharp_1.default)(buf).rotate().resize(200, 200, { fit: "inside", withoutEnlargement: true }).webp({ quality: WEBP_Q }).toBuffer(),
        (0, sharp_1.default)(buf).rotate().resize(500, 500, { fit: "inside", withoutEnlargement: true }).webp({ quality: WEBP_Q }).toBuffer(),
    ]);
...
function parseUpload(name) {
    let m = name.match(/^igrejas\/([^/]+)\/membros\/([^/]+)\/foto_perfil\.(jpg|jpeg|png|webp)$/i);
    if (m)
