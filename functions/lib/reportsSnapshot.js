...
exports.getRelatoriosBundle = functions
     ...
     */
    const readByDate = async (collection, dateField, max) => {
        try {
            let q = churchRef.collection(collection);
            if (from)
                q = q.where(dateField, ">=", from);
            if (to)
                q = q.where(dateField, "<=", to);
    ...
    if (modules.includes("finance")) {
        tasks.push(readByDate("finance", "date", financeLimit).then(async (docs) => {
            if (docs.length > 0) {
