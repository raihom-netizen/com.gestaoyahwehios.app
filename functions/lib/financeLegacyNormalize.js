...
exports.normalizeChurchFinanceLancamentos = void 0;
exports.buildFinanceNormalizationPatch = buildFinanceNormalizationPatch;
...
async function callerCanNormalize(uid, tokenRole, tokenTenantId, tenantId) {
    ...
    if (["MASTER", "ADMIN", "ADM"].includes(role))
        return true;
...
function normalizedType(d) {
    ...
    if (raw === "income" || raw === "expense")
        return raw;
    if (raw.includes("entrada") || raw.includes("receita"))
        return "income";
    if (raw.includes("saida") || raw.includes("saída") || raw.includes("despesa")) {
        return "expense";
    }
    if (raw === "transferencia" || raw === "transferência")
        return null;
...
function timestampFromCompetencia(v) {
...
function buildFinanceNormalizationPatch(d) {
    const patch = {};
    ...
    if (!String(d.status ?? "").trim()) {
        const pendente = d.pendenteConciliacaoRecorrencia === true ||
...
exports.normalizeChurchFinanceLancamentos = functions
    .region("us-central1")
    ...
    const allowed = await callerCanNormalize(context.auth.uid, token?.role, token?.igrejaId ?? token?.tenantId, tenantId);
    if (!allowed) {
        throw new functions.https.HttpsError("permission-denied", "Sem permissão para normalizar o financeiro desta igreja.");
    }
...
//# sourceMappingURL=financeLegacyNormalize.js.map