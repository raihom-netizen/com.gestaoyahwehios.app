...
async function isMasterPanelActor(
  uid: string,
  tokenRole: unknown,
  email: string,
): Promise<boolean> {
  const role = String(tokenRole ?? "").trim().toLowerCase();
  if (role === "master" || role === "adm" || role === "admin") return true;
...
export type MigrationModule = "membros" | "avisos" | "eventos" | "patrimonio" | "all";
...
function pickHttpUrl(data: Record<string, unknown>, keys: string[]): string {
  for (const k of keys) {
    const v = data[k];
...
async function memberSourceCandidates(
  tenantId: string,
  memberId: string,
  ...
  if (auth && auth !== mid) stems.add(auth);

  const paths: string[] = [];
  for (const s of stems) {
    paths.push(
      `igrejas/${tid}/membros/fotos/${s}.webp`,
      `igrejas/${tid}/membros/${s}/foto_perfil.webp`,
      `igrejas/${tid}/membros/${s}/foto_perfil.jpg`,
      `igrejas/${tid}/membros/${s}/foto_perfil.jpeg`,
      `igrejas/${tid}/membros/${s}/foto_perfil.png`,
      `igrejas/${tid}/membros/${s}.jpg`,
    );
  }
...
async function resolveMemberSourceBuffer(
  tenantId: string,
  memberId: string,
  data: Record<string, unknown>,
): Promise<Buffer | null> {
...
async function migrateMemberDoc(
  tenantId: string,
  memberId: string,
  data: Record<string, unknown>,
  dryRun: boolean,
  ...
  if (hasFull && hasThumb && hasThumbUrl) return false;
...
async function migratePatrimonioDoc(
  tenantId: string,
  itemId: string,
  dryRun: boolean,
): Promise<number> {
  ...
  for (let slot = 0; slot < 5; slot++) {
    ...
    const [fullBuf, thumbBuf] = await Promise.all([
      sharp(src).rotate().resize(1920, 1920, { fit: "inside", withoutEnlargement: true }).webp({ quality: 78 }).toBuffer(),
      sharp(src).rotate().resize(200, 200, { fit: "cover" }).webp({ quality: 70 }).toBuffer(),
    ]);
...
async function migrateLegacyFeedFolder(
  tenantId: string,
  module: "avisos" | "eventos",
  dryRun: boolean,
): Promise<number> {
  const tid = tenantId.trim();
  ...
  for (const file of files) {
    ...
    const aviso = name.match(
      new RegExp(`^igrejas/${tid}/avisos/([^/]+)/(capa_aviso|galeria_\\d+)\\.(jpe?g|png|webp)$`, "i"),
    );
    const evento = name.match(
      new RegExp(`^igrejas/${tid}/eventos/([^/]+)/(banner_evento|galeria_\\d+)\\.(jpe?g|png|webp)$`, "i"),
    );
    ...
    const base = m[2];
    const suffix =
      base === "capa_aviso" || base === "banner_evento"
...
async function migrateEventVideoThumbs(tenantId: string, dryRun: boolean): Promise<number> {
  const tid = tenantId.trim();
  ...
  for (const file of files) {
    ...
    const out = await sharp(buf).rotate().resize(200, 200, { fit: "cover" }).webp({ quality: 70 }).toBuffer();
    await saveWebp(dest, out);
    migrated += 1;
  }
  return migrated;
...
async function listTenantIds(allTenants: boolean, tenantId?: string): Promise<string[]> {
...
export async function runStorageConsolidationMigration(
  options: StorageMigrationOptions,
): Promise<StorageMigrationResult> {
  const dryRun = options.dryRun !== false;
  const modules = options.modules?.length ? options.modules : (["all"] as MigrationModule[]);
    ...
    try {
      if (runAll || modules.includes("membros")) {
        ...
        if (!dryRun) {
          ...
          await recomputeMembersDirectoryFromDocs(tid, refreshSnap.docs, refreshSnap.size);
...
export const migrateStorageConsolidated = functions
  .region("us-central1")
  ...
  .https.onCall(async (request, context) => {
    ...
    if (!context.auth || !(await isMasterPanelActor(context.auth.uid, context.auth.token?.role, email))) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Apenas operador do painel master.",
      );
    }
    ...
    const modules: MigrationModule[] = Array.isArray(modulesRaw)
      ? (modulesRaw.map(String) as MigrationModule[])
      : ["all"];
