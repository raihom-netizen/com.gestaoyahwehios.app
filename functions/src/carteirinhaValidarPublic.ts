...
export type CarteirinhaValidacaoPayload = {
  ok: boolean;
  found: boolean;
  active: boolean;
  churchName: string;
...
function normalizePublicSiteBase(church: Record<string, unknown>): string {
  for (const k of [
    "customPublicDomain",
    "publicSiteDomain",
    "dominioPublico",
...
function churchPublicFormattedAddress(data: Record<string, unknown>): string {
  ...
  const cidade = str(
    data.cidade ?? data.CIDADE ?? data.localidade ?? data.LOCALIDADE,
  );
  ...
  const lista: string[] = [];
  if (ruaCompleta) lista.push(ruaCompleta);
  if (bairro) lista.push(bairro);
  if (cidadeEstado) lista.push(cidadeEstado);
  if (cep) lista.push(`CEP ${cep}`);
  if (lista.length > 0) return lista.join(", ");

  return str(data.endereco ?? data.ENDERECO);
}

function churchPublicPhoneRaw(data: Record<string, unknown>): string {
...
function churchPublicHomeUrl(
  ...
  if (!slug) return base === PUBLIC_WEB_BASE ? `${PUBLIC_WEB_BASE}/` : base;
  return `${base.replace(/\/$/, "")}/${encodeURIComponent(slug)}`;
}

function churchLogoUrl(church: Record<string, unknown>): string {
...
function pastorFromMember(m: Record<string, unknown>): {
  name: string;
  role: string;
} {
...
export function buildChurchPublicInfo(
  church: Record<string, unknown>,
  tenantId: string,
  ...
  if (!pastorRole) pastorRole = "Pastor(a)";
...
export function parseCarteirinhaQueryParams(
  raw: Record<string, unknown>,
): { tenantId: string; memberKey: string } {
...
async function resolveTenantDocId(tenantKey: string): Promise<string> {
...
async function findMemberSnapshot(
  tenantId: string,
  memberKey: string,
  ...
  if (cpfDigits.length === 11) {
    for (const field of ["CPF", "cpf"]) {
      ...
      if (!qFmt.empty) return qFmt.docs[0];
    }
  }
...
export async function validateCarteirinhaCore(
  tenantKey: string,
  memberKey: string,
): Promise<CarteirinhaValidacaoPayload> {
...
function infoRow(icon: string, label: string, value: string, href?: string): string {
  if (!value) return "";
  const inner = href
...
function renderChurchBlock(ch: ChurchPublicInfo): string {
...
function renderCarteirinhaHtml(result: CarteirinhaValidacaoPayload): string {
  ...
  if (result.titularMascarado) {
    memberRows.push(
      `<div class="member-pill"><span class="pill-label">Titular</span><span class="pill-value">${escapeHtml(result.titularMascarado)}</span></div>`,
    );
  }
  if (result.validityHint) {
    memberRows.push(
      `<div class="member-pill"><span class="pill-label">Validade</span><span class="pill-value">${escapeHtml(result.validityHint)}</span></div>`,
    );
  }
 ...
 */
export const carteirinhaValidarHttp = functions
  .region("us-central1")
