...
function canEmitCarteirinhaRole(role: string): boolean {
...
async function downloadStoragePathBuffer(storagePath: string): Promise<Buffer | null> {
...
async function qrPngBuffer(text: string): Promise<Buffer | null> {
  try {
...
function cargoFromMemberData(md: admin.firestore.DocumentData): string {
  const c = String(
    md.cargo_lideranca || md.CARGO_LIDERANCA || md.cargo || md.CARGO || "",
  ).trim();
  return c || "Membro";
}

function departamentoFromMemberData(md: admin.firestore.DocumentData): string {
  const d = String(
    md.departamento ||
      md.DEPARTAMENTO ||
      md.department ||
...
function carteiraValidityLabel(md: admin.firestore.DocumentData): string {
  const perm =
    md.CARTEIRA_PERMANENTE === true ||
...
function certificadoValidationUrl(certificadoId: string): string {
  const cid = String(certificadoId || "").trim();
  return `${PUBLIC_WEB_BASE}/#/validar?cid=${encodeURIComponent(cid)}`;
}

function carteirinhaValidationUrl(tenantId: string, memberId: string): string {
...
async function loadCertificateConfig(
  ...
  if (!textoModelo) textoModelo = TEXTO_MODELO_PADRAO[templateId] || TEXTO_MODELO_PADRAO.batismo;
  if (!titulo) titulo = TITULO_PADRAO[templateId] || "Certificado";
  ...
  const useDigital =
    String(certData.defaultSignatureMode || "digital").trim() !== "manual";
...
export const gerarCertificadoPdf = functions
  .region("us-central1")
...
export const gerarCarteirinhaPdf = functions
  .region("us-central1")
  .runWith({ timeoutSeconds: 120, memory: "512MB" })
  .https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    if (!photo) {
      const photoUrl = String(
        md.foto_url || md.fotoUrl || md.photoURL || md.photoUrl || "",
      ).trim();
      if (photoUrl) photo = await downloadUrlBuffer(photoUrl);
    }
