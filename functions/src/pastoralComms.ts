...
async function canSendChurchCommunications(
  uid: string,
  tokenRole: unknown,
  tokenTenantId: unknown,
  tenantId: string
): Promise<boolean> {
  if (await canManageTenant(uid, tokenRole, tokenTenantId, tenantId)) return true;
  const role = await resolveRoleFromTokenOrDb(uid, tokenRole);
  if (!PASTORAL_COMMS_ROLES.has(role)) return false;
  const tokenTenant = String(tokenTenantId || "").trim();
  if (tokenTenant && tokenTenant === tenantId) return true;
  try {
    const u = await db.collection("users").doc(uid).get();
...
export function slugTopicPart(raw: string): string {
  const s = String(raw || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
function ymdStringInTz(d: Date, tz: string): string {
...
async function collectFcmTokensForCpfs(tenantId: string, cpfs: string[]): Promise<string[]> {
...
async function firstNameForCpf(tenantId: string, cpfDigits: string): Promise<string> {
  const col = db.collection("igrejas").doc(tenantId).collection("membros");
  const digits = cpfDigits.replace(/\D/g, "");
  if (digits.length !== 11) return "Irmão(ã)";
  let snap = await col.doc(digits).get();
  if (!snap.exists) {
    const q = await col.where("CPF", "==", digits).limit(1).get();
    snap = q.docs[0] || snap;
  }
  if (!snap.exists) return "Irmão(ã)";
  const full = String(
    (snap.data() || {}).NOME_COMPLETO || (snap.data() || {}).nome || ""
  ).trim();
  return (full.split(/\s+/)[0] || "Irmão(ã)").replace(/,$/, "");
}

/** E-mail do cadastro de membro (para notificações HTML). */
async function memberEmailFromMembro(tenantId: string, cpfDigits: string): Promise<string | null> {
  const col = db.collection("igrejas").doc(tenantId).collection("membros");
  const digits = cpfDigits.replace(/\D/g, "");
...
async function runMultiSegmentDelivery(params: {
  ...
  const {
    ...
    segment: segIn,
    departmentId,
    departmentIds: deptIdsIn,
    cargoLabel,
    cargoLabels: cargosIn,
    memberDocId,
    memberDocIds: membersIn,
  } = params;
  const segment = (segIn || "broadcast").toLowerCase();

  const deptList =
    deptIdsIn.length > 0 ? deptIdsIn : departmentId ? [departmentId] : [];
  const cargoList =
    cargosIn.length > 0 ? cargosIn : cargoLabel ? [cargoLabel] : [];
  const memberList =
    membersIn.length > 0 ? membersIn : memberDocId ? [memberDocId] : [];

  const baseData = (seg: string): Record<string, string> => ({
    tenantId,
    type: "pastoral_comm",
  ...
  if (segment === "member") {
    ...
    for (const mid of memberList) {
      ...
      if (cpfRaw.length === 11) cpfs.push(cpfRaw);
    }
    if (!cpfs.length) {
      throw new functions.https.HttpsError(
        "failed-precondition",
        "Nenhum membro válido (CPF) na seleção."
      );
    }
    ...
    if (!tokens.length) {
      throw new functions.https.HttpsError(
        "failed-precondition",
        "Nenhum aparelho com notificação para os membros selecionados."
      );
    }
...
export const sendSegmentedPush = functions.region("us-central1").https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  if (expiresAt) msgPayload.expiresAt = expiresAt;
  ...
  if (expiresAt) notifPayload.expiresAt = expiresAt;
...
export const archivePastoralMessage = functions.region("us-central1").https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Faça login.");
  }
  const tenantId = String(data?.tenantId || "").trim();
...
export const resendDevotionalEnvio = functions.region("us-central1").https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Faça login.");
  }
  const tenantId = String(data?.tenantId || "").trim();
...
export const deleteDevotionalEnvio = functions.region("us-central1").https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Faça login.");
  }
  const tenantId = String(data?.tenantId || "").trim();
...
async function userCpfDigitsFromUid(uid: string): Promise<string> {
  try {
    const u = await db.collection("users").doc(uid).get();
    const data = u.data() || {};
...
function cpfsFromDepartmentData(data: Record<string, unknown> | undefined): string[] {
  const out: string[] = [];
  const add = (raw: unknown) => {
    const x = String(raw || "").replace(/\D/g, "");
    if (x.length === 11 && !out.includes(x)) out.push(x);
  };
  if (!data) return out;
  const rawList = data.leaderCpfs ?? data.leader_cpfs;
  if (Array.isArray(rawList)) for (const e of rawList) add(e);
  add(data.leaderCpf);
...
async function isDepartmentLeaderCpf(
  tenantId: string,
  departmentId: string,
  cpfDigits: string
): Promise<boolean> {
  ...
  return cpfsFromDepartmentData(snap.data() as Record<string, unknown>).includes(cpfDigits);
}

async function canNotifyScheduleForUser(
  uid: string,
  tokenRole: unknown,
  tokenTenantId: unknown,
  tenantId: string,
  ...
  if (await canSendChurchCommunications(uid, tokenRole, tokenTenantId, tenantId)) return true;
  const cpf = await userCpfDigitsFromUid(uid);
  if (!departmentId || cpf.length !== 11) return false;
 ...
 */
export const notifySchedulePublished = functions.region("us-central1").https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Faça login.");
  }
  const tenantId = String(data?.tenantId || "").trim();
  ...
  const allowed = await canNotifyScheduleForUser(
    context.auth.uid,
    context.auth.token?.role,
    context.auth.token?.igrejaId || context.auth.token?.tenantId,
    tenantId,
  ...
  const locLine =
    String(d.location || d.local || d.place || d.setor || "").trim() ||
    deptName ||
    "—";
  ...
  for (const cpf of memberCpfs) {
    ...
    const body = `${nome}, você foi escalado(a) para ${deptName || "o ministério"} no dia ${dateStr}${
      titleStr ? ` — ${titleStr}` : ""
    }`
      .replace(/\s+/g, " ")
      .trim()
  ...
  try {
    for (const cpf of memberCpfs) {
      ...
      const { subject, html } = buildEscalaEmail({
      ...
      const sent = await sendGestaoYahwehHtmlEmail({ to: email, subject, html });
...
export const onEscalaImpedimentoNotifyLeaders = functions
  ...
  .onUpdate(async (change, context) => {
    ...
    const escId = context.params.escId as string;
    const before = (change.before.data() || {}) as Record<string, unknown>;
    const after = (change.after.data() || {}) as Record<string, unknown>;
    const befConf = (before.confirmations || {}) as Record<string, unknown>;
    const aftConf = (after.confirmations || {}) as Record<string, unknown>;
    ...
    for (const k of Object.keys(aftConf)) {
      ...
      if (v !== "indisponivel") continue;
      const prev = String(befConf[k] || "").trim();
      if (prev === "indisponivel") continue;
      const cpfNorm = k.replace(/\D/g, "");
      if (cpfNorm.length === 11) newlyUnavailable.push(cpfNorm);
    }
    if (!newlyUnavailable.length) return null;

    const ts = after.date as admin.firestore.Timestamp | undefined;
    const eventDate = ts && typeof ts.toDate === "function" ? ts.toDate() : null;
...
function parseMemberBirthDate(data: Record<string, unknown>): Date | null {
  const keys = [
    "DATA_NASCIMENTO",
    "dataNascimento",
    "birthDate",
...
export const dailyBirthdayTopicPush = functions
  ...
  .onRun(async () => {
    ...
    if (!monthToday || !dayToday) return null;
    ...
    for (const ch of churches.docs) {
      ...
      try {
        ...
        if (last === todaySp) continue;

        const names: string[] = [];
        const birthdayEmailRecipients: { email: string; displayName: string }[] = [];
        const col = db.collection("igrejas").doc(tenantId).collection("membros");
        ...
        for (;;) {
          ...
          const membros = lastDoc
            ...
            : await base.get();
          if (membros.empty) break;
          for (const doc of membros.docs) {
            const d = doc.data() || {};
            const status = String(d.STATUS || d.status || "")
              .trim()
              .toLowerCase();
            if (status.includes("inativ") || status.includes("bloq")) continue;
          ...
          if (membros.docs.length < 500) break;
        }

        if (names.length === 0) {
          await stateRef.set(
            { lastSentYmd: todaySp, skippedEmpty: true, at: admin.firestore.FieldValue.serverTimestamp() },
            { merge: true }
          );
          continue;
        }
        ...
        try {
          for (const r of birthdayEmailRecipients) {
            const { subject, html } = buildAniversarianteEmail({
              nomeDestinatario: r.displayName,
            });
            const ok = await sendGestaoYahwehHtmlEmail({ to: r.email, subject, html });
            if (ok) birthdayEmailsSent += 1;
          }
        } catch (e) {
 ...
 */
export const dayBeforeScaleReminder = functions
  .region("us-central1")
  .pubsub.schedule("15 8 * * *")
  .timeZone(TZ_BR)
  .onRun(async () => {
    functions.logger.info(
      "dayBeforeScaleReminder: no-op — usar rollingScaleRemindersConfirmed (confirmados, 24h e 1h)."
    );
    return null;
  });
...
function confirmedScheduleCpfsForReminders(d: Record<string, unknown>): string[] {
  ...
  const conf = (d.confirmations || {}) as Record<string, unknown>;
  const out: string[] = [];
...
function eventUtcMsSp(dateTs: admin.firestore.Timestamp, timeStr: string): number | null {
      ...
      try {
        ...
        for (const doc of escSnap.docs) {
          ...
          for (const cpf of cpfs) {
            ...
            const nome = await firstNameForCpf(tenantId, cpf);
            if (send24) {
              const body = `${nome}, em cerca de 24h você tem escala confirmada (${dateStr}${
                timeStr ? ` às ${timeStr}` : ""
              })${dept ? ` — ${dept}` : ""}`
                .replace(/\s+/g, " ")
                .trim()
            ...
            if (send1) {
              const body = `${nome}, sua escala confirmada começa em cerca de 1 hora${
                timeStr ? ` (${timeStr})` : ""
              }${dept ? ` — ${dept}` : ""}${titleStr ? ` · ${titleStr}` : ""}`
                .replace(/\s+/g, " ")
                .trim()
                .slice(0, 220);
              for (const token of tokens) {
 ...
 */
export const hourlyDevotionalBroadcast = functions
  .region("us-central1")
  .pubsub.schedule("5 * * * *")
  .timeZone(TZ_BR)
  .onRun(async () => {
    ...
    for (const t of tenants.docs) {
      ...
      try {
        ...
        if (c.devocionalEnabled !== true) continue;
        const hourCfg = Number(c.devocionalHora ?? 7);
        if (!Number.isFinite(hourCfg) || hourCfg < 0 || hourCfg > 23) continue;
        if (hourSp !== hourCfg) continue;
        ...
        await cfgSnap.ref.set(
          { devocionalUltimoEnvioDia: todaySp, devocionalUltimoEnvioAt: admin.firestore.FieldValue.serverTimestamp() },
          { merge: true }
        );
      } catch (e) {
...
function normCpfDigitsSwap(raw: string): string {
  return String(raw || "").replace(/\D/g, "");
}

function remapCpfKeyedMapSwap(oldMap: Record<string, unknown>, newCpfs: string[]): Record<string, unknown> {
...
async function fullNameForCpfSwap(tenantId: string, cpfDigits: string): Promise<string> {
  const col = db.collection("igrejas").doc(tenantId).collection("membros");
 ...
 */
export const respondScheduleSwap = functions.region("us-central1").https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Faça login.");
  }
  const tenantId = String(data?.tenantId || "").trim();
  ...
  if (!accept) {
    ...
    const declBody = `${await firstNameForCpf(tenantId, alvo)} recusou o pedido de troca de escala.`
      ...
      .trim()
      .slice(0, 200);
    const declMsgs: admin.messaging.Message[] = solTokens.map((token) =>
      buildGyTokenMessage({
        token,
  ...
  await db.runTransaction(async (tx) => {
    ...
    const cpfs = ((ed.memberCpfs as unknown[]) || []).map((e) => String(e));
    const names = ((ed.memberNames as unknown[]) || []).map((e) => String(e));
    ...
    while (newNames.length < newCpfs.length) newNames.push("");
    newNames[idx] = alvoNomeResolved;

    const oldConf = { ...((ed.confirmations as Record<string, unknown>) || {}) };
    const oldUnav = { ...((ed.unavailabilityReasons as Record<string, unknown>) || {}) };
...
export const onEscalaTrocaInviteTarget = functions
  ...
  .onCreate(async (snap, context) => {
    ...
    const body = `${solPrimeiro} pediu para você assumir a escala${dateStr ? ` (${dateStr})` : ""}${
      titleStr ? ` — ${titleStr}` : ""
    }. Abra o app em Minhas Escalas para aceitar ou recusar.`
      .replace(/\s+/g, " ")
      .trim()
      .slice(0, 220);
    ...
    if (tokens.length) {
      const msgs: admin.messaging.Message[] = tokens.map((token) =>
        buildGyTokenMessage({
          token,
          ...
          data: {
            ...
            trocaId: context.params.trocaId as string,
            click_action: "FLUTTER_NOTIFICATION_CLICK",
          },
          module: "escala",
        })
    ...
    if (creatorUid.length >= 8) {
      ...
      const creatorTokens = (await collectFcmTokensForUids([creatorUid])).filter(
        (t) => !alvoToks.has(t) && !solToks.has(t)
      );
      if (creatorTokens.length) {
        const alvoPrimeiro = await firstNameForCpf(tenantId, alvo);
        const bodyCreator = `${solPrimeiro} pediu troca com ${alvoPrimeiro} — ${
          titleStr ? `${titleStr} ` : ""
        }${dateStr ? `(${dateStr})` : ""}.`
          .replace(/\s+/g, " ")
          .trim()
