...
async function zipBuffers(files: { name: string; buffer: Buffer }[]): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    for (const f of files) {
      if (f.buffer.length > 0) archive.append(f.buffer, { name: f.name });
    }
    void archive.finalize();
  });
}

export const gerarCertificadosEmLote = functions
  .region("us-central1")
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!tenantId || memberIds.length === 0) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "tenantId e memberIds são obrigatórios"
      );
    }
    if (memberIds.length > 150) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Máximo 150 certificados por lote"
      );
    }
    if (roleCaller !== "MASTER" && callerTenantId !== tenantId) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Sem permissão para outra igreja"
      );
    }
    ...
    const sigSnaps = await Promise.all(
      idsToLoad.map((mid) => db.doc(`igrejas/${tenantId}/membros/${mid}`).get())
    );
    ...
    if (useDigital) {
      const sigUrls: { url: string }[] = [];
      for (const mSnap of sigSnaps) {
        if (!mSnap.exists) continue;
        const md = mSnap.data() || {};
        const raw = String(md.assinaturaUrl || md.assinatura_url || "").trim();
        if (raw) sigUrls.push({ url: raw });
      }
      const bufs = await Promise.all(
        sigUrls.map((s) => downloadUrlBuffer(s.url))
      );
    ...
    const CHUNK = 10;
    const allMemberDocs: (admin.firestore.DocumentSnapshot | null)[] = new Array(memberIds.length).fill(null);
    ...
    for (let idx = 0; idx < memberIds.length; idx++) {
      ...
      if (!mSnap || !mSnap.exists) continue;
      const md = mSnap.data() || {};
    ...
    if (pdfFiles.length === 0) {
      throw new functions.https.HttpsError(
        "not-found",
        "Nenhum membro encontrado para os IDs informados"
      );
    }
