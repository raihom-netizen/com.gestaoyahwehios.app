 ...
 */
export const purgeChurchChatMessagesAdmin = functions
  .region("us-central1")
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!canManageChurchChatPurge(roleCaller)) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Só gestor, pastor ou administrador pode limpar todo o chat."
      );
    }
    ...
    for (const threadDoc of threadsSnap.docs) {
      ...
      while (hasMore) {
        ...
        for (const msg of batch.docs) {
          const d = msg.data() as { storagePath?: string };
        ...
        if (batch.size < 400) hasMore = false;
      }
