...
export async function collectRelatedIgrejaDocIds(seed: string): Promise<string[]> {
  ...
  for (const suf of ["_sistema", "_bpc"]) {
    if (raw.endsWith(suf)) ids.add(raw.slice(0, -suf.length));
    else ids.add(`${raw}${suf}`);
  }
      ...
      for (const field of ["slug", "alias", "slugId", "churchId"]) {
        ...
        for (const f of ["slug", "alias", "slugId"]) {
          try {
            ...
            for (const x of q.docs) ids.add(x.id);
          } catch {
            /* índice opcional */
          }
        }
...
async function scoreTenantData(tenantId: string): Promise<number> {
...
async function copyCollectionIfMissing(
  sourceId: string,
  targetId: string,
  collectionId: string,
): Promise<number> {
...
async function copyConfigDocs(sourceId: string, targetId: string): Promise<string[]> {
  const copied: string[] = [];
  for (const docId of CONFIG_DOCS) {
    try {
      const from = await db()
        ...
        .doc(docId)
        .get();
      if (!from.exists || !from.data()) continue;
      const dest = db()
        .collection("igrejas")
        .doc(targetId)
        .collection("config")
        .doc(docId);
      const existing = await dest.get();
      if (existing.exists && Object.keys(existing.data() || {}).length > 0) continue;
...
export async function runSyncChurchClusterDataFromRichest(
  tenantId: string,
  ...
  await metaRef.set(
    { status: "running", sourceTenantId: bestSource, startedAt: admin.firestore.FieldValue.serverTimestamp() },
    { merge: true },
  );
...
export const syncChurchClusterDataFromRichest = functions
  .region("us-central1")
