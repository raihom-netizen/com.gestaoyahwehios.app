# Migração escalas — popula escalados[] + memberUids (denormalização legado → rápido).
# Requer Admin SDK (conta de serviço ou gcloud application-default login).
#
# Simular (padrão — não grava):
...
#
# Executar gravação (pedido explícito):
...
#
# Reprocessar docs que já têm escalados[] parcial:
#   .\scripts\migrate_escalas_escalados_denorm.ps1 -Execute -Force

param(
  [string]$Igreja = "",
  [switch]$Execute,
  [switch]$Force,
  [switch]$NoConfirmMirror
)
...
try {
  ...
  $nodeArgs = @("scripts/migrate-escalas-escalados-denorm.js")
  if ($Igreja.Trim()) {
  ...
  if ($NoConfirmMirror) { $nodeArgs += "--no-confirm-mirror" }
