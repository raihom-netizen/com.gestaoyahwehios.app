// Força atualização SEGURA para 11.2.305+2170.
// - Web: webRefresh=true -> a aba recarrega para o build novo (já live).
// - Mobile: PRESERVA minBuildNumber atual (NÃO tranca quem ainda não tem 2170
//   nas lojas Play/App Store; o AAB/iOS ainda vão ser publicados/revisados).
//   Depois que as lojas tiverem 2170, sobe-se minBuildNumber pelo painel.
...
(async () => {
  ...
  // Preserva minBuildNumber (mobile) — nunca sobe para 2170 aqui.
  ...
  const payload = {
    ...
    webRefresh: true,                       // <- força a WEB a recarregar
  ...
  console.log('OK: web forçado para', FULL, '| minBuildNumber mobile preservado em', keepMinBuild);
  process.exit(0);
