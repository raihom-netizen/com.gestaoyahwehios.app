#!/usr/bin/env python3
"""
Ativa Push Notifications (PUSH_NOTIFICATIONS) no App ID via App Store Connect API.
Sem esta capability, o perfil App Store nasce sem aps-environment e o IPA com
UIBackgroundModes=remote-notification é rejeitado («Binário inválido»).
"""
...
def _exact_bundle_id(token: str, base: str) -> Optional[str]:
    """Match EXATO — filter ASC é prefixo (Widget vinha antes do app)."""
    ...
    # fallback primeiro resultado (legado)
    data = bundle_resp.get("data") or []
    if data:
        print(
            f"AVISO: match exacto falhou; a usar primeiro resultado "
...
def _main_impl() -> int:
    if not ISSUER or not KEY_ID:
        print("AVISO: APP_STORE_CONNECT_ISSUER_ID ou KEY_IDENTIFIER vazio — a saltar Push API.")
        return 0
    ...
    if code2 == 200:
        for item in detail.get("included") or []:
            ...
            if attr.get("capabilityType") == CAP:
                print(f"OK: {CAP} já está ativo no App ID {BUNDLE_FILTER}.")
                return 0
    ...
    if code3 in (200, 201):
        ...
        return 0
    if code3 == 409:
        print(f"OK: capability {CAP} já existia (409).")
        return 0
    print(f"AVISO: POST bundleIdCapabilities HTTP {code3}: {post_resp}")
    print(
        "Dica: ative Push Notifications em developer.apple.com → Identifiers → "
        f"{BUNDLE_FILTER} → Push Notifications."
    )
    return 0
...
def main() -> int:
    try:
        ...
        print(f"AVISO: Push Notifications (API) não concluído: {e}")
        return 0
