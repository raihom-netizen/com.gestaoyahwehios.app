...
A('# ---------------------------------------------------------------------------')
A('# Disparo: SOMENTE manual — Actions > iOS TestFlight > Run workflow, ou')
A('#   gh workflow run ios_testflight.yml')
A('# Nao ha gatilho por push: build iOS so acontece quando alguem pede.')
...
A('          if [ -z "$IPA" ]; then IPA=$(ls -1 build/ios/ipa/*.ipa 2>/dev/null | head -1); fi')
A('          if [ -z "$IPA" ]; then echo "::error::nenhum .ipa encontrado para enviar"; exit 1; fi')
A('          # Mesmo script de envio dos outros apps: 3 tentativas e 90189 tratado.')
A('          GHA_IOS_IPA_PATH="$IPA" bash scripts/gha_ios_publish_testflight.sh .')
A('')
