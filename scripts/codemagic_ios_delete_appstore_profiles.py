"""
Remove perfis IOS_APP_STORE do App Store Connect para bundle(s) do app.
Usado no Codemagic antes de fetch-signing-files para forçar recriação com capabilities atuais
(Sign In with Apple, Push, App Groups no Widget, etc.).
"""
import json
import os
import subprocess
...
def _name_hints() -> tuple[str, ...]:
    return (
        "GestaoYahwehBootstrap",
        "Gestao Yahweh",
        "GestaoYahwehWidget",
        "gestaoyahwehiosapp",
...
def main() -> int:
    ...
    for item in data or []:
        ...
        name_hit = any(h in name for h in hints)
        if ident not in targets and not (ident is None and name_hit):
            continue
        print("Removendo perfil App Store:", name, pid, f"(bundle={ident})")
        subprocess.run(
    ...
    print("Bundles alvo:", ", ".join(sorted(targets)))
    return 0
